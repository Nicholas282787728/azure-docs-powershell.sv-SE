...
## PROBLEMBESKRIVNING
Cmdleten **set-AzureRmResource** ändrar en befintlig Azure-resurs.
...
Det första kommandot får resursen som heter ContosoSite med hjälp av Get-AzureRmResource cmdlet och lagrar den i $Resource-variabeln.
Det andra kommandot ändrar en egenskap för $Resource.
Det sista kommandot uppdaterar resursen så att den matchar $Resource.

### Exempel 2: ändra alla resurser i en viss resurs grupp
```
...
Det första kommandot får resurserna i resurs gruppen testrg och lagrar dem sedan i $Resource variabel.

Det andra kommandot upprepar de olika resurserna i resurs gruppen och lägger till en ny tagg till dem.

Det sista kommandot uppdaterar var och en av dessa resurser.
...
Anger namnet på en tilläggs resurs för resursen.
...
Om tilläggs resursen till exempel är en databas anger du följande: `Microsoft.Sql/Servers/Databases`
...
### -InputObject
Objekt representation för resursen som ska uppdateras.
...
### -Planera
Anger resurs Plans egenskaper som en hash-tabell för resursen.
...
### -Egenskaper
Anger resurs egenskaper för resursen.
...
### -ResourceGroupName
Anger namnet på resurs gruppen där denna cmdlet ändrar resursen.
...
### -SKU
Anger SKU-objekt för resursen som en hash-tabell.
...
### -UsePatchSemantics
Anger att denna cmdlet använder en HTTP-korrigering för att uppdatera objektet, i stället för ett HTTP-PUT.
