...
Använd cmdlet "New-AzFrontDoorHeaderActionObject" för att skapa PSHeaderObjects till parametrarna "-RequestHeaderActions" och "-ResponseHeaderActions".
...
Skapa en regel motor åtgärd och visa hur du visar egenskaperna för regel åtgärds åtgärden skapad.

## MALLPARAMETRAR

### -BackendPoolName
Namnet på den BackendPool som den här regeln dirigerar till
...
Frågesträngen måste vara i \<key\> = \<value\> formatet.
Den första?
& läggs till automatiskt, så ta inte med dem på fram sidan, men dela upp flera frågesträngar med &.
...
### -DynamicCompression
Om du vill aktivera dynamisk komprimering för cachelagrat innehåll.
...
### -EnableCaching
Om cachelagring ska aktive ras för den här vägen.
...
### -ForwardingProtocol
Protokollet som den här regeln ska använda när trafik vidarebefordras till bakände.
...
### -RedirectProtocol
Protokollet för mål platsen där trafiken omdirigeras.
Standardvärdet är MatchRequest
...
### -RedirectType
Omdirigerings typ regeln används vid omdirigering av trafik.
...
### -RequestHeaderAction
En lista med rubrik åtgärder som ska utföras från AFD till ursprunget.
...
En lista med rubrik åtgärder som ska gälla från svaret från AFD till klienten.
