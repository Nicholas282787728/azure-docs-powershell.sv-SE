...
## PROBLEMBESKRIVNING
Cmdleten **set-AzSynapseIntegrationRuntime** uppdaterar en integrations körning med specifika parametrar.
...
Cmdleten lägger till arbets ytan för att använda den delade integrerings körningen. Om du använder en `-SharedIntegrationRuntimeResourceId` parameter `-Type` måste också inkluderas. Observera att arbets ytan måste ges tillstånd att använda integrerings programmet innan cmdlet körs.
...
Namnet på den Self-Hosted-integrering som används som proxy.
...
Sökvägen i mellanlagringsområdet för att användas när data flyttas mellan Self-Hosted och Azure-SSIS integrerings körningar används en standard behållare om ospecificerad.
...
### -ExpressCustomSetup
Den snabba anpassade konfigurationen för SSIS integration runtime som kan användas för att konfigurera konfigurationer och komponenter från tredje part utan anpassade installations skript.
...
### -LicenseType
Den licens typ som du vill välja för SSIS IR.
Det finns två typer: LicenseIncluded eller BasePrice.
Om du är berättigad till priser för Azure Hybrid användning (AHUB) väljer du BasePrice.
Om inte, Välj LicenseIncluded.
...
### -VNetId
ID för det VNet som integrerings körningen ska anslutas till.
