...
## Sammanfattning
Ställer in principen från en klient organisation i Azure Attestationn.
...
## PROBLEMBESKRIVNING
Den Set-AzAttestationPolicy cmdleten ställer in policyn från en klient organisation i Azure-attestering.
...
Anger den användardefinierade principen för shirt (typ *SgxEnclave* för attesteringsservern *pshtest* med ett text policy format (standard).
...
Anger den användardefinierade principen för shirt (typ *SgxEnclave* för attesteringsservern *PSHTEST* med ett JWT-princip-format.
...
### -Namn
Anger ett namn på innehavaren.
Den här cmdleten ställer in policyn för attestering för den klient organisation som den här parametern anger.
...
### -Princip
Anger det princip dokument som ska ställas in.  Princip formatet kan antingen vara text eller JSON Web token (JWT).
...
### -PolicyFormat
Anger formatet för principen, antingen text eller JWT (JSON Web token).  Standard princip formatet är text.
...
### -Shirt (
Anger en typ av körnings miljö.
Det finns fyra typer av miljöer: SgxEnclave, OpenEnclave, CyResComponent och VBSEnclave.
