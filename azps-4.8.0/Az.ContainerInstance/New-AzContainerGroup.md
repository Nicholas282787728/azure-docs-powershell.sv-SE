...
## PROBLEMBESKRIVNING
Cmdleten **New-AzContainerGroup** skapar en behållar grupp.
...
Med de här kommandona skapas en behållar grupp med den senaste nginx-bilden och begär en offentlig IP-adress med öppning av port 8000
...
De här kommandona skapar en behållar grupp och kör ett anpassat skript i behållaren.

### Exempel 3: skapa en behållare för en slut för ande tjänst.
```
...
De här kommandona skapar en behållar grupp som skriver ut ' Hej ' och slutar.

### Exempel 4: skapar en behållar grupp med hjälp av bild i Azure-behållarobjekt
```
...
Det här kommandot skapar en behållar grupp med hjälp av en nginx-bild i Azure Container-registret.

### Exempel 5: skapar en behållar grupp med hjälp av bild i eget register för behållar bilder
```
PS C:\> $secpasswd = ConvertTo-SecureString "PlainTextPassword" -AsPlainText -Force
...
Med de här kommandona skapas en behållar grupp med hjälp av en anpassad bild från ett eget register för en behållare.

### Exempel 6: skapar en behållar grupp som monterar Azure-filvolymen
```
PS C:\> $secpasswd = ConvertTo-SecureString "PlainTextPassword" -AsPlainText -Force
PS C:\> $mycred = New-Object System.Management.Automation.PSCredential ("username", $secpasswd)
...
De här kommandona skapar en behållar grupp som monterar den tillhandahållna Azure-fildelningen `/mnt/azfile` .
...
Det här kommandot skapar en behållar grupp med en tilldelad identitet med den senaste nginx-avbildningen och begär en offentlig IP-adress med öppning av port 8000
...
Med de här kommandona skapas en behållar grupp med tilldelad och användardefinierad identitet med den senaste nginx-avbildningen och begär en offentlig IP-adress med öppning av port 8000.

## MALLPARAMETRAR

### -AssignIdentity
Aktivera tilldelad identitet
...
Autentiseringsuppgifterna för lagrings kontot för Azure-fildelningen för att montera där användar namnet är lagrings kontots namn och nycklar är lagrings kontots fil.
...
Monterings vägen för Azure-filvolymen.
...
Namnet på den Azure-fildelning du vill montera.
...
### -Kommando
Kommandot som ska köras i behållaren.
...
### -DnsNameLabel
DNS-namnets etikett för IP-adressen.
...
### -IdentityId
Användarens tilldelade ID-nummer
...
### -IdentityType
Den hanterade identitets typen
...
### -Port
De portar som ska öppnas. Standard: [80]
...
### -RegistryCredential
Den anpassade autentiseringsuppgiften för container-registret.
...
Den anpassade inloggnings servern för en behållare.
...
### -RestartPolicy
Restart-principen för container. Standard: alltid
