...
### Exempel 1: skapa en händelse källa för eventhub under den angivna miljön
```powershell
...
Det här kommandot skapar en händelse källa för eventhub under den angivna miljön.

### Exempel 2: skapa en iothub-händelse källa under den angivna miljön
```powershell
...
Det här kommandot skapar en iothub-händelse under den angivna miljön.

## MALLPARAMETRAR

### -ConsumerGroupName
Namnet på den konsument grupp för evenemanget/IoT Hub som innehåller de partitioner som händelser kommer att läsas ifrån.
...
Resurs-ID för händelse källan i Azure Resource Manager.
...
### -Namn
Namnet på den SAS-nycklar som beviljar åtkomst till Event/IoT-navet för tids serie.
...
### -ServiceBusNameSpace
Namnet på den tjänst buss som innehåller händelsehubben.
...
### -SharedAccessKey
Värdet på den delade åtkomst-tangenten som beviljar tjänsten Time Series Insights Läs åtkomst till evenemanget/IoT Hub.
...
Händelse egenskapen som kommer att användas som händelse källans tidsstämpel.
