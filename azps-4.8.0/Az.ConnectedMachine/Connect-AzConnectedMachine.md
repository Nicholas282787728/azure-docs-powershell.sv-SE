...
## Sammanfattning
API för att registrera en ny dator och därmed skapa en spårad resurs i armar
...
### Exempel 1: informerar datorn om att du är ansluten till datorn
```powershell
...
Informerar datorn om att du är ansluten till datorn.

### Exempel 2: inbyggda en fjärrdator som ansluten enhet
```powershell
...
Hanterar en fjärrdator som en ansluten enhet med PowerShell-fjärrkommunikation.
Obs! endast Windows som mål stöder just nu.
...
### -Namn
Det namn som ska användas för den här datorn.
Värd namnet används som standard.
...
### -Proxy
URI för proxyservern som ska användas
...
### -PSSession
När det här alternativet anges körs kommandot som inbyggda maskiner till Azure i varje PSSession.
...
### -ResourceGroupName
Namnet på resurs gruppen där du vill lägga till datorn.
...
### -SubscriptionId
ID för den prenumeration där du vill lägga till datorn.
