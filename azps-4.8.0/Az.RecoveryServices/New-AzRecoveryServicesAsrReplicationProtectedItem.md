...
Startar åtgärden för att skapa replikerat objekt för det angivna systemfilerna för automatisk system återställning inklusive selektiva diskar och returnerar ASR-jobbet som används för att spåra åtgärden (vmWare to Azure scenario) med valda diskar.
...
Startar åtgärden Skapa replikerat objekt för det angivna VmId och returnerar det ASR-jobb som används för att spåra åtgärden (Azure to Azure-scenariot). För den information om redundans som skickades till cmdlet för kryptering används den.
...
Startar åtgärden Skapa replikerat objekt för en virtuell dator inom närhet och returnerar det ASR-jobb som används för att spåra åtgärden (Azure to Azure-scenariot).
...
### -AzureToAzure
Switch parameter anger att det replikerade objektet ska skapas i Azure-scenario.
...
Anger den disk konfiguration som används för att använda den virtuella datorn för Azure till Azure katastrof återställnings scenario.
...
### -AzureVmId
Anger ID för Azure VM för katastrof återställnings skydd i återställnings området.
...
### -DiskEncryptionSetId
Anger resurs-ID för disk krypterings uppsättningen som ska användas för kryptering av de hanterade diskarna.
...
### -DiskType
Anger den hanterade disk typen för återställning av virtuell dator.
...
Anger disk konfigurations inmatning för vMWare-disk-ID för att skydda från ett specifikt skyddat objekt.
...
### -KeyEncryptionKeyUrl
Anger URL-adressen till den disk krypterings plats med version (Azure Disk Encryption) som ska användas för återställning av virtuell dator efter redundans.
...
Anger den disk krypterings nycklar-valv-ID (Azure Disk Encryption) som ska användas för återställning av virtuell dator efter redundans.
...
Anger tillgänglighets zonen för återställning av virtuell dator efter redundans.
...
Ange grupp-ID för närhet som används av den virtuella dator för mål återställning.
