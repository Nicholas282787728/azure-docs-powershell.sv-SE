...
## PROBLEMBESKRIVNING
Kommandot **Invoke-AzSynapsePipeline** startar en körning på den angivna pipeline och returnerar ett ID för den. Detta GUID kan överföras till **Get-AzSynapsePipelineRun** eller **Get-AzSynapseActivityRun** för att få mer information om den här körningen.
...
Det här kommandot startar en körning för pipeline som heter ContosoPipeline i arbets ytans ContosoWorkspace.
...
Det här kommandot startar en körning för pipeline som heter ContosoPipeline i arbets ytans ContosoWorkspace via pipeline.
...
### -InputObject
Information om pipeline-körningen.
...
### -IsRecovery
Flagga för återställnings läge.
Om återställnings läget är inställt på True körs den angivna åberopade pipelinen och den nya körningen grupperas under samma inställningar.
...
Kör-ID för pipeline för kör igen.
Om kör-ID anges används parametrarna för den angivna körningen för att skapa ett nytt kör.
...
### -StartActivityName
I återställnings läget startas kör igen med den här aktiviteten.
Om det inte anges körs alla aktiviteter.
