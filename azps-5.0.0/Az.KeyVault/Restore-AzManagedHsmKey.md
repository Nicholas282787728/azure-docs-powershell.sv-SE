...
## Sammanfattning
Skapar en nycklar i en hanterad HSM från en säkerhets kopia.
...
## PROBLEMBESKRIVNING
Cmdleten **restore-AzManagedHsmKey** skapar en Key i den angivna hanterade HSM.
Den här tangenten är en replik av säkerhets kopian i indatafilen och har samma namn som den ursprungliga tangenten.
Om den hanterade HSM redan har en nycklar med samma namn, Miss lyckas denna cmdlet i stället för att skriva över den ursprungliga tangenten.
Om säkerhets kopian innehåller flera versioner av en, återställs alla versioner.
Den hanterade HSM som du återställer tangenten till kan skilja sig från den hanterade HSM som du säkerhetskopierade från.
Däremot måste hanterad HSM använda samma abonnemang och vara i ett Azure-område på samma geografi (till exempel Nord Amerika).
...
Det här kommandot återställer en-och alla dess versioner från säkerhets kopian med namnet Backup. blob till den hanterade HSM som heter testmhsm.
