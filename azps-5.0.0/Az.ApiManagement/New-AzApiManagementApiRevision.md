...
Cmdleten **New-AzApiManagementApiRevision** skapar en API-revision för en befintlig API i API-hanteringskonsolen.
...
Det här kommandot skapar en API-revision `2` av `echo-api` API: t.
...
Det här kommandot skapar en API-revision `5` av `echo-api` API: t.

## MALLPARAMETRAR

### -ApiId
ID för API vars ändring ska skapas.
...
### -ApiRevisionDescription
Beskrivning av API-revision. Denna parameter är valfri.
...
### -ServiceUrl
En URL för webb tjänsten som visar API i backend-tjänsten. Denna URL kommer endast att användas av Azure API Management och blir inte offentlig. Måste vara 1 till 2000 tecken långt. Denna parameter är obligatorisk.
...
### -SourceApiRevision
API-version för käll-API. Denna parameter är valfri.
