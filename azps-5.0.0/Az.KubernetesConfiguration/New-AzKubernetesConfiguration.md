...
### Exempel 1: skapa en configuation för Kubernetes-kluster
```powershell
...
Det här kommandot skapar en configuation för Kubernetes-kluster.
...
### -ClusterScoped
Om du har angett en konfiguration för klustret (standard namn område).
...
### -EnableHelmOperator
Alternativ för att aktivera Helm-operator för denna git-konfiguration.
...
Värden åsidosätter för Helm-diagrammet.
...
Instans namn för operatören som identifierar den specifika konfigurationen.
...
### -OperatorNamespace
Namn området som den här operatorn är installerad till.
Högst 253 gemener alfanumeriska tecken, bindestreck och punkt.
...
### -OperatorParameters
Alla parametrar för Operator instansen i sträng format.
...
### -RepositoryUrl
URL-adressen till SourceControl-databasen.
