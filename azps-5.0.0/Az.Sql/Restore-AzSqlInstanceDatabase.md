...
## PROBLEMBESKRIVNING
Cmdleten **restore-AzSqlInstanceDatabase** återställer en instans databas från en Geo-redundant säkerhets kopiering, en tidpunkt i en Live-databas eller en långsiktig säkerhets kopiering.
Den återställda databasen skapas som en ny instans databas.

## BESKRIVS

### Exempel 1: Återställ en instans databas från en tidpunkt
```
...
### Exempel 3: Geo-Restore en instans databas
```
...
Det andra kommandot återställer säkerhets kopian i $GeoBackup till instans databasen med namnet Database01_restored.

### Exempel 4: återställa en borttagen instans databas från en tidpunkt
```
...
Det första kommandot får de borttagna instans databaserna "DB1" på instansen "managedInstance1".
Det andra kommandot återställer den hämtade databasen, från den angivna säkerhets kopian under tiden till instans databasen med namnet Database01_restored.

### Exempel 5: återställa en borttagen instans databas från en tidpunkt
```
PS C:\> $deletedDatabase = Get-AzSqlDeletedInstanceDatabaseBackup -ResourceGroupName "ResourceGroup01" -InstanceName "managedInstance1" -DatabaseName "DB1"
...
Det första kommandot får de borttagna instans databaserna "DB1" på instansen "managedInstance1".
Det andra kommandot återställer den hämtade databasen, från den angivna säkerhets kopian under tiden till instans databasen med namnet Database01_restored med hjälp av indata.

### Exempel 6: återställa en databas från vä-hö-säkerhetskopiering.
```
...
Återställer vä-hö-säkerhetskopiering med angivet resurs-ID (som du kan hitta genom att köra Get-AzSqlInstanceDatabaseLongTermRetentionBackup).
...
### -DeletionDate
Borttagnings datumet för den borttagna databasen.
...
### -FromGeoBackup
Återställ från en geo-säkerhetskopia.
...
Återställ från en långsiktig säkerhets kopiering.
...
### -GeoBackupObject
Det återställnings bara instans databas objekt som ska återställas
