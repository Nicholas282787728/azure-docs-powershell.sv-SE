...
## PROBLEMBESKRIVNING
Cmdleten **New-AzPrivateDnsVirtualNetworkLink** skapar en ny virtuell DNS-länk (Domain Name System) i den angivna resurs gruppen och den privata zonen. Du måste ange ett unikt länknamn för parametern *namn* , eller så returnerar cmdleten ett fel. Du kan använda *Confirm* -parametern och $ConfirmPreference Windows PowerShell-variabel för att kontrol lera om cmdleten uppmanar dig att bekräfta.

## BESKRIVS

### Exempel 1: skapa ett privat virtuellt DNS-nätverk.
```
...
Det här kommandot skapar en ny virtuell nätverks länk som är kopplad till den privata DNS-zon som heter myzone.com och Virtual Network "myvirtualnetwork" (som redan har skapats i resurs gruppen) i den angivna resurs gruppen och sedan lagrar den i $Link variabel.
...
### -EnableRegistration
Byt parameter som representerar om länken är aktive rad eller inte.
...
### -Namn
Anger namnet på den virtuella nätverks länk som ska skapas.
...
### -RemoteVirtualNetworkId
Resurs-ID för det virtuella nätverket i en annan klient organisation.
...
### -ResourceGroupName
Anger den resurs grupp som länken ska skapas i.
...
### -VirtualNetwork
Det virtuella nätverks objekt som är kopplat till länken.
...
### -VirtualNetworkId
Resurs-ID för det virtuella nätverk som är kopplat till länken.
...
### -Zonnamn
Anger namnet på den zon som ska länkas till länken virtuellt nätverk.
