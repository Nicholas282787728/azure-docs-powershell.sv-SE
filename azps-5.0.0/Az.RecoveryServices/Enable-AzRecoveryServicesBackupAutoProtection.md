...
Den andra cmdleten hämtar den relevanta SQLInstance som är ett skyddat objekt. Det tredje kommandot ställer in automatisk skydd för den här instansen med principen i $Pol.

### Exempel 2

Med de här kommandona kan användarna automatiskt skydda alla befintliga oskyddade DBs och alla databaser som läggs till senare med den angivna principen. (automatiskt genererat)
...
Den klass med resurser som skyddas. För närvarande kan de värden som stöds för denna cmdlet vara monoklonal, AzureWorkload, AzureVM
...
### -InputItem
Anger det skydd bara objekt som kan överföras som indata. Det aktuella värdet som stöds är ett protectableItem-objekt av typen "SQLInstance". 
...
### -WorkloadType
Arbets belastnings typ för resursen. De aktuella värdena är AzureVM, WindowsServer, MSSQL
