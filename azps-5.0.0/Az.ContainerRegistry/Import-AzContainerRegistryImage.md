...
Importera upptaget till ACR. Fotnot 
* "bibliotek/" måste läggas till före käll bild. "upptagen: senaste" => "biblioteket/upptagen: senaste"
* Autentiseringsuppgifter krävs om käll registret inte är offentligt tillgängligt
...
Importera upptaget från source ACR till Target ACR. Fotnot 
* Autentiseringsuppgifter krävs om administratörs användaren för käll registret hade inaktiverats.
...
### -Mode
När Force används skrivs alla befintliga målattribut ut.
Om det inte går att utföra operationen när kopieringen påbörjas Miss lyckas eventuella befintliga målattribut.
...
### -Lösen ord
Lösen ordet som används för att autentisera med käll registret.
...
### -SourceImage
Käll bildens databas namn.

Ange en bild per databas (' Hej-världen ').
Detta kommer att använda märket "senaste".

Ange en bild efter tagg (' Hej-världen: senaste ").

Ange en bild med SHA256 manifest sammandrag (' hello-world@sha256:abc123 ').
...
Resurs-ID för käll-Azure Container-registret.
...
### -SourceRegistryUri
Adressen till käll registret (till exempel "mcr.microsoft.com").
...
### -TargetTag
Lista över strängar i formen repo \[ : tagg \] .
När tagg utelämnas används källan (eller "senaste" om käll märke också utelämnas).
...
Lista över strängar för databas namn för att endast kopiera en manifest kopia.
Ingen tagg skapas.
...
### -Username
Användar namnet som autentiseras med käll registret.
