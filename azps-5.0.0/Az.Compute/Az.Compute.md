---
Module Name: Az.Compute
Module Guid: 0a83c907-1ffb-4d87-a492-c65ac7d7ed37
Download Help Link: https://docs.microsoft.com/en-us/powershell/module/az.compute
Help Version: 3.5.0.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Compute/Compute/help/Az.Compute.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Compute/Compute/help/Az.Compute.md
ms.openlocfilehash: e0cb2feccbafae5d914047bcddcf9e3eb6933e9d
ms.sourcegitcommit: b4a38bcb0501a9016a4998efd377aa75d3ef9ce8
ms.translationtype: MT
ms.contentlocale: sv-SE
ms.lasthandoff: 10/27/2020
ms.locfileid: "94323399"
---
# AZ. Compute-modul
## Problembeskrivning
I det här avsnittet visas hjälp avsnitten för Azure Compute cmdlets.

## AZ. Compute-cmdletar
### [Add-AzContainerServiceAgentPoolProfile](Add-AzContainerServiceAgentPoolProfile.md)
Lägger till en cacheprofil för en behållar tjänst.

### [Add-AzImageDataDisk](Add-AzImageDataDisk.md)
Lägger till en datadisk till ett bild objekt.

### [Add-AzVhd](Add-AzVhd.md)
Uppladdar en virtuell hård disk från en lokal virtuell dator till en BLOB i ett moln lagrings konto i Azure.

### [Add-AzVMAdditionalUnattendContent](Add-AzVMAdditionalUnattendContent.md)
Lägger till information i svars filen för obevakad Windows-installation.

### [Add-AzVMDataDisk](Add-AzVMDataDisk.md)
Lägger till en datadisk till en virtuell dator.

### [Add-AzVMNetworkInterface](Add-AzVMNetworkInterface.md)
Lägger till ett nätverks gränssnitt till en virtuell dator.

### [Add-AzVMSecret](Add-AzVMSecret.md)
Lägger till en hemlighet till en virtuell dator.

### [Add-AzVmssAdditionalUnattendContent](Add-AzVmssAdditionalUnattendContent.md)
Lägger till information i svars filen för obevakad Windows-installation.

### [Add-AzVmssDataDisk](Add-AzVmssDataDisk.md)
Lägger till en datadisk till VMSS.

### [Add-AzVmssDiagnosticsExtension](Add-AzVmssDiagnosticsExtension.md)
Lägger till ett diagnostikverktyg i VMSS.

### [Add-AzVmssExtension](Add-AzVmssExtension.md)
Lägger till en anknytning till VMSS.

### [Add-AzVMSshPublicKey](Add-AzVMSshPublicKey.md)
Lägger till de offentliga nycklarna för SSH för en virtuell dator när det bara skapas.

### [Add-AzVmssNetworkInterfaceConfiguration](Add-AzVmssNetworkInterfaceConfiguration.md)
Lägger till en nätverks gränssnitts konfiguration i VMSS.

### [Add-AzVmssSecret](Add-AzVmssSecret.md)
Lägger till en hemlighet till en VMSS.

### [Add-AzVmssSshPublicKey](Add-AzVmssSshPublicKey.md)
Lägger till offentliga nycklar för SSH till VMSS.

### [Add-AzVmssVMDataDisk](Add-AzVmssVMDataDisk.md)
Lägger till en datadisk till en VMSS VM.

### [Add-AzVmssWinRMListener](Add-AzVmssWinRMListener.md)
Lägger till en WinRM-lyssnare till VMSS.

### [ConvertTo-AzVMManagedDisk](ConvertTo-AzVMManagedDisk.md)
Konverterar en virtuell dator med blobbbaserade diskar till en virtuell dator med hanterade diskar.

### [Disable-AzVMDiskEncryption](Disable-AzVMDiskEncryption.md)
Inaktiverar kryptering på en IaaS virtuell dator.

### [Disable-AzVmssDiskEncryption](Disable-AzVmssDiskEncryption.md)
Inaktiverar disk kryptering på en virtuell dators skala.

### [Exportera-AzLogAnalyticRequestRateByInterval](Export-AzLogAnalyticRequestRateByInterval.md)
Exportera loggar som visar API-begäranden som gjorts av detta abonnemang i tidsfönstret för att Visa begränsnings aktiviteter.

### [Exportera-AzLogAnalyticThrottledRequest](Export-AzLogAnalyticThrottledRequest.md)
Exportera loggar som visar totalt antal begränsade API-begäranden för detta abonnemang i det angivna tidsfönstret.

### [Get-AzAvailabilitySet](Get-AzAvailabilitySet.md)
Hämtar Azure Availability-uppsättningar i en resurs grupp.

### [Get-AzComputeResourceSku](Get-AzComputeResourceSku.md)
Lista alla Compute Resource-SKU: er

### [Get-AzContainerService](Get-AzContainerService.md)
Hämtar en behållar tjänst.

### [Get-AzDisk](Get-AzDisk.md)
Hämtar egenskaperna för en hanterad disk.

### [Get-AzDiskAccess](Get-AzDiskAccess.md)
Hämtar egenskaper för disk åtkomst

### [Get-AzDiskEncryptionSet](Get-AzDiskEncryptionSet.md)
Hämta eller Visa disk krypterings uppsättningar.

### [Get-AzGallery](Get-AzGallery.md)
Hämta eller lista gallerier.

### [Get-AzGalleryImageDefinition](Get-AzGalleryImageDefinition.md)
Bild definitionerna hämta eller lista Galleri.

### [Get-AzGalleryImageVersion](Get-AzGalleryImageVersion.md)
Bild versioner för hämta eller lista

### [Get-AzHost](Get-AzHost.md)
Hämta eller Visa värdar.

### [Get-AzHostGroup](Get-AzHostGroup.md)
Hämta eller Visa värdar.

### [Get-AzImage](Get-AzImage.md)
Hämtar egenskaperna för en bild.

### [Get-AzProximityPlacementGroup](Get-AzProximityPlacementGroup.md)
Få eller Visa en lista över närhets resurser.

### [Get-AzRemoteDesktopFile](Get-AzRemoteDesktopFile.md)
Hämtar en. RDP-fil.

### [Get-AzSnapshot](Get-AzSnapshot.md)
Hämtar egenskaperna för en ögonblicks bild

### [Get-AzVM](Get-AzVM.md)
Hämtar egenskaperna för en virtuell dator.

### [Get-AzVMAccessExtension](Get-AzVMAccessExtension.md)
Hämtar information om VMAccess-tillägget.

### [Get-AzVMADDomainExtension](Get-AzVMADDomainExtension.md)
Hämtar information om ett AD-domännätverk.

### [Get-AzVMAEMExtension](Get-AzVMAEMExtension.md)
Hämtar information om AEM-tillägget.

### [Get-AzVMBootDiagnosticsData](Get-AzVMBootDiagnosticsData.md)
Hämtar startdiagnostik-data för en virtuell dator.

### [Get-AzVMChefExtension](Get-AzVMChefExtension.md)
Hämtar information om en kock-anknytning.

### [Get-AzVMCustomScriptExtension](Get-AzVMCustomScriptExtension.md)
Hämtar information om ett anpassat skript tillägg.

### [Get-AzVMDiagnosticsExtension](Get-AzVMDiagnosticsExtension.md)
Hämtar inställningarna för tillägget diagnostik på en virtuell dator.

### [Get-AzVMDiskEncryptionStatus](Get-AzVMDiskEncryptionStatus.md)
Hämtar krypterings statusen för den virtuella datorn.

### [Get-AzVMDscExtension](Get-AzVMDscExtension.md)
Hämtar inställningarna för DSC-tillägget på en viss virtuell dator.

### [Get-AzVMDscExtensionStatus](Get-AzVMDscExtensionStatus.md)
Hämtar statusen för DSC-filtillägg för en virtuell dator.

### [Get-AzVMExtension](Get-AzVMExtension.md)
Hämtar egenskaper för virtuella dator tillägg som är installerade på en virtuell dator.

### [Get-AzVMExtensionImage](Get-AzVMExtensionImage.md)
Hämtar alla versioner för ett Azure-tillägg.

### [Get-AzVMExtensionImageType](Get-AzVMExtensionImageType.md)
Hämtar typen av ett Azure-tillägg.

### [Get-AzVMImage](Get-AzVMImage.md)
Får alla versioner av en VMImage.

### [Get-AzVMImageOffer](Get-AzVMImageOffer.md)
Hämtar typer av VMImage-offerter.

### [Get-AzVMImagePublisher](Get-AzVMImagePublisher.md)
Hämtar VMImage-utgivaren.

### [Get-AzVMImageSku](Get-AzVMImageSku.md)
Får VMImage SKU: er.

### [Get-AzVMRunCommandDocument](Get-AzVMRunCommandDocument.md)
Hämta ett kommando dokument för kör.

### [Get-AzVMSize](Get-AzVMSize.md)
Hämtar tillgängliga storlekar på virtuell dator.

### [Get-AzVMSqlServerExtension](Get-AzVMSqlServerExtension.md)
Hämtar inställningarna för ett SQL Server-tillägg på en virtuell dator.

### [Get-AzVmss](Get-AzVmss.md)
Hämtar egenskaperna för en VMSS.

### [Get-AzVmssDiskEncryption](Get-AzVmssDiskEncryption.md)
Visar disk krypterings statusen för en virtuell dators skala.

### [Get-AzVmssRollingUpgrade](Get-AzVmssRollingUpgrade.md)
Visar status för den senaste virtuella dator skalnings den rullande uppgraderingen.

### [Get-AzVmssSku](Get-AzVmssSku.md)
Hämtar de tillgängliga SKU: erna för VMSS.

### [Get-AzVmssVM](Get-AzVmssVM.md)
Hämtar egenskaperna för en VMSS-virtuell dator.

### [Get-AzVmssVMDiskEncryption](Get-AzVmssVMDiskEncryption.md)
Visar disk krypterings statusen för virtuella datorer i en virtuell dators skala.

### [Get-AzVMUsage](Get-AzVMUsage.md)
Hämtar användning för antal virtuella datorer för en plats.

### [Grant-AzDiskAccess](Grant-AzDiskAccess.md)
Ger åtkomst till en disk.

### [Grant-AzSnapshotAccess](Grant-AzSnapshotAccess.md)
Ger åtkomst till en ögonblicks bild.

### [Invoke-AzVMPatchAssessment](Invoke-AzVMPatchAssessment.md)
Bedöm uppdaterings status för en virtuell dator.

### [Invoke-AzVMReimage](Invoke-AzVMReimage.md)
Bild av en virtuell Azure-dator.

### [Invoke-AzVMRunCommand](Invoke-AzVMRunCommand.md)
Kör ett kommando på den virtuella datorn.

### [Invoke-AzVmssVMRunCommand](Invoke-AzVmssVMRunCommand.md)
Kommandot kör på den virtuella datorns skala ange virtuell dator.

### [New-AzAvailabilitySet](New-AzAvailabilitySet.md)
Skapar en Azure Availability-uppsättning.

### [New-AzContainerService](New-AzContainerService.md)
Skapar en behållar tjänst.

### [New-AzContainerServiceConfig](New-AzContainerServiceConfig.md)
Skapar ett lokalt konfigurations objekt för en behållar tjänst.

### [New-AzDisk](New-AzDisk.md)
Skapar en hanterad disk.

### [New-AzDiskAccess](New-AzDiskAccess.md)
Skapar en disk åtkomst resurs

### [New-AzDiskConfig](New-AzDiskConfig.md)
Skapar ett konfigurerbart disk objekt.

### [New-AzDiskEncryptionSet](New-AzDiskEncryptionSet.md)
Skapar en disk krypterings uppsättning.

### [New-AzDiskEncryptionSetConfig](New-AzDiskEncryptionSetConfig.md)
Skapar ett konfigurerbart objekt för disk krypterings uppsättningen.

### [New-AzDiskUpdateConfig](New-AzDiskUpdateConfig.md)
Skapar ett konfigurerbart disk uppdaterings objekt.

### [New-AzGallery](New-AzGallery.md)
Skapa ett galleri.

### [New-AzGalleryImageDefinition](New-AzGalleryImageDefinition.md)
Skapa en bild definition för galleriet.

### [New-AzGalleryImageVersion](New-AzGalleryImageVersion.md)
Skapa en galleri bild version.

### [New-AzHost](New-AzHost.md)
Skapar en värd.

### [New-AzHostGroup](New-AzHostGroup.md)
Skapar en värd grupp.

### [New-AzImage](New-AzImage.md)
Skapar en bild.

### [New-AzImageConfig](New-AzImageConfig.md)
Skapar ett konfigurerbart bild objekt.

### [New-AzProximityPlacementGroup](New-AzProximityPlacementGroup.md)
Skapa en grupp resurs för närhets placering.

### [New-AzSnapshot](New-AzSnapshot.md)
Skapar en ögonblicks bild.

### [New-AzSnapshotConfig](New-AzSnapshotConfig.md)
Skapar ett konfigurerbart SnapShot-objekt.

### [New-AzSnapshotUpdateConfig](New-AzSnapshotUpdateConfig.md)
Skapar ett konfigurerbart uppdaterings objekt för ögonblicks bilder.

### [New-AzVM](New-AzVM.md)
Skapar en virtuell dator.

### [New-AzVMConfig](New-AzVMConfig.md)
Skapar ett konfigurerbart virtuellt dator objekt.

### [New-AzVMDataDisk](New-AzVMDataDisk.md)
Skapar ett lokalt data disk objekt för en virtuell dator eller en VMSS VM.

### [New-AzVMSqlServerAutoBackupConfig](New-AzVMSqlServerAutoBackupConfig.md)
Skapar ett konfigurations objekt för automatisk säkerhets kopiering av SQL Server.

### [New-AzVMSqlServerAutoPatchingConfig](New-AzVMSqlServerAutoPatchingConfig.md)
Skapar ett konfigurations objekt för automatisk korrigering på en virtuell dator.

### [New-AzVMSqlServerKeyVaultCredentialConfig](New-AzVMSqlServerKeyVaultCredentialConfig.md)
Skapar ett konfigurations objekt för SQL Server Key valv-autentiseringsuppgifter på en virtuell dator.

### [New-AzVmss](New-AzVmss.md)
Skapar en VMSS.

### [New-AzVmssConfig](New-AzVmssConfig.md)
Skapar ett VMSS.

### [New-AzVmssIpConfig](New-AzVmssIpConfig.md)
Skapar en IP-konfiguration för ett nätverks gränssnitt för en VMSS.

### [New-AzVmssIpTagConfig](New-AzVmssIpTagConfig.md)
Skapar ett IP-taggattribut för ett nätverks gränssnitt för en VMSS.

### [New-AzVmssVaultCertificateConfig](New-AzVmssVaultCertificateConfig.md)
Skapar en konfiguration för ett nyckelord för valv.

### [Publicera – AzVMDscConfiguration](Publish-AzVMDscConfiguration.md)
Laddar upp ett DSC-skript till Azure Blob Storage.

### [Remove-AzAvailabilitySet](Remove-AzAvailabilitySet.md)
Tar bort en tillgänglighets uppsättning från Azure.

### [Remove-AzContainerService](Remove-AzContainerService.md)
Tar bort en behållar tjänst.

### [Remove-AzContainerServiceAgentPoolProfile](Remove-AzContainerServiceAgentPoolProfile.md)
Tar bort en cacheprofil från en behållar tjänst.

### [Remove-AzDisk](Remove-AzDisk.md)
Tar bort en disk.

### [Remove-AzDiskAccess](Remove-AzDiskAccess.md)
Tar bort en disk åtkomst resurs.

### [Remove-AzDiskEncryptionSet](Remove-AzDiskEncryptionSet.md)
Tar bort en disk krypterings uppsättning.

### [Remove-AzGallery](Remove-AzGallery.md)
Ta bort ett galleri.

### [Remove-AzGalleryImageDefinition](Remove-AzGalleryImageDefinition.md)
Ta bort en bild definition för galleriet.

### [Remove-AzGalleryImageVersion](Remove-AzGalleryImageVersion.md)
Ta bort en galleri bild version.

### [Remove-AzHost](Remove-AzHost.md)
Tar bort en värd.

### [Remove-AzHostGroup](Remove-AzHostGroup.md)
Tar bort en värd grupp.

### [Remove-AzImage](Remove-AzImage.md)
Tar bort en bild.

### [Remove-AzImageDataDisk](Remove-AzImageDataDisk.md)
Tar bort en datadisk från ett bild objekt.

### [Remove-AzProximityPlacementGroup](Remove-AzProximityPlacementGroup.md)
Ta bort resursen för närhets placering.

### [Remove-AzSnapshot](Remove-AzSnapshot.md)
Tar bort en stillbild.

### [Remove-AzVM](Remove-AzVM.md)
Tar bort en virtuell dator från Azure.

### [Remove-AzVMAccessExtension](Remove-AzVMAccessExtension.md)
Tar bort VMAccess-tillägget från en virtuell dator.

### [Remove-AzVMAEMExtension](Remove-AzVMAEMExtension.md)
Tar bort AEM-tillägget från en virtuell dator.

### [Remove-AzVMBackup](Remove-AzVMBackup.md)
Tar bort säkerhets kopian från en virtuell dator.

### [Remove-AzVMChefExtension](Remove-AzVMChefExtension.md)
Tar bort kock-tillägget från en virtuell dator.

### [Remove-AzVMCustomScriptExtension](Remove-AzVMCustomScriptExtension.md)
Tar bort ett anpassat skript tillägg från en virtuell dator.

### [Remove-AzVMDataDisk](Remove-AzVMDataDisk.md)
Tar bort en datadisk från en virtuell dator.

### [Remove-AzVMDiagnosticsExtension](Remove-AzVMDiagnosticsExtension.md)
Tar bort tillägget diagnostik från en virtuell dator.

### [Remove-AzVMDiskEncryptionExtension](Remove-AzVMDiskEncryptionExtension.md)
Tar bort disk krypterings tillägget från en virtuell dator.

### [Remove-AzVMDscExtension](Remove-AzVMDscExtension.md)
Tar bort en DSC-tilläggsprovider från en virtuell dator i en resurs grupp.

### [Remove-AzVMExtension](Remove-AzVMExtension.md)
Tar bort ett tillägg från en virtuell dator.

### [Remove-AzVMNetworkInterface](Remove-AzVMNetworkInterface.md)
Tar bort ett nätverks gränssnitt från en virtuell dator.

### [Remove-AzVMSecret](Remove-AzVMSecret.md)
Tar bort (a) hemliga (a) hemligheter från ett virtuellt dator objekt

### [Remove-AzVMSqlServerExtension](Remove-AzVMSqlServerExtension.md)
Tar bort ett SQL Server-tillägg från en virtuell dator.

### [Remove-AzVmss](Remove-AzVmss.md)
Tar bort VMSS eller en virtuell dator i VMSS.

### [Remove-AzVmssDataDisk](Remove-AzVmssDataDisk.md)
Tar bort en datadisk från VMSS.

### [Remove-AzVmssDiagnosticsExtension](Remove-AzVmssDiagnosticsExtension.md)
Tar bort ett Diagnostics-tillägg från VMSS.

### [Remove-AzVmssExtension](Remove-AzVmssExtension.md)
Tar bort ett tillägg från VMSS.

### [Remove-AzVmssNetworkInterfaceConfiguration](Remove-AzVmssNetworkInterfaceConfiguration.md)
Tar bort en konfiguration för nätverks gränssnitt från en VMSS.

### [Remove-AzVmssVMDataDisk](Remove-AzVmssVMDataDisk.md)
Tar bort en datadisk från en virtuell dator skala ange virtuell dator

### [Reparera-AzVmssServiceFabricUpdateDomain](Repair-AzVmssServiceFabricUpdateDomain.md)
Manuell uppdatering av plattforms uppdateringar för att uppdatera virtuella datorer i en skala för virtuell dator med en tjänst infrastruktur.

### [Restart-AzVM](Restart-AzVM.md)
Startar om en virtuell Azure-dator.

### [Restart-AzVmss](Restart-AzVmss.md)
Startar om VMSS eller en virtuell dator i VMSS.

### [Återkalla-AzDiskAccess](Revoke-AzDiskAccess.md)
Återkallar en åtkomst till en disk.

### [Återkalla-AzSnapshotAccess](Revoke-AzSnapshotAccess.md)
Återkallar en åtkomst till en ögonblicks bild.

### [Spara – AzVhd](Save-AzVhd.md)
Sparar hämtade VHD-bilder lokalt.

### [Spara – AzVMImage](Save-AzVMImage.md)
Sparar en virtuell dator som en VMImage.

### [Set-AzDiskDiskEncryptionKey](Set-AzDiskDiskEncryptionKey.md)
Anger egenskaper för disk krypterings nycklar på ett disk objekt.

### [Set-AzDiskImageReference](Set-AzDiskImageReference.md)
Anger bild referens egenskaperna för ett disk objekt.

### [Set-AzDiskKeyEncryptionKey](Set-AzDiskKeyEncryptionKey.md)
Anger egenskaper för Key Encryption Key på ett disk objekt.

### [Set-AzDiskUpdateDiskEncryptionKey](Set-AzDiskUpdateDiskEncryptionKey.md)
Anger egenskaper för disk krypterings nycklar på ett disk uppdaterings objekt.

### [Set-AzDiskUpdateKeyEncryptionKey](Set-AzDiskUpdateKeyEncryptionKey.md)
Anger egenskaperna för Key Encryption Key för ett disk uppdaterings objekt.

### [Set-AzImageOsDisk](Set-AzImageOsDisk.md)
Ställer in disk egenskaper för operativ systemet på ett bild objekt.

### [Set-AzSnapshotDiskEncryptionKey](Set-AzSnapshotDiskEncryptionKey.md)
Ställer in egenskaper för disk krypterings nycklar i ett SnapShot-objekt.

### [Set-AzSnapshotImageReference](Set-AzSnapshotImageReference.md)
Ställer in bild referens egenskaperna för ett SnapShot-objekt.

### [Set-AzSnapshotKeyEncryptionKey](Set-AzSnapshotKeyEncryptionKey.md)
Anger egenskaperna för Key Encryption Key för ett SnapShot-objekt.

### [Set-AzSnapshotUpdateDiskEncryptionKey](Set-AzSnapshotUpdateDiskEncryptionKey.md)
Ställer in egenskaper för disk krypterings nycklar i ett objekt för ögonblicks bilder.

### [Set-AzSnapshotUpdateKeyEncryptionKey](Set-AzSnapshotUpdateKeyEncryptionKey.md)
Anger egenskaperna för Key Encryption Key för ett ögonblicks bild uppdaterings objekt.

### [Set-AzVM](Set-AzVM.md)
Markerar en virtuell dator som allmänt.

### [Set-AzVMAccessExtension](Set-AzVMAccessExtension.md)
Lägger till VMAccess-tillägget på en virtuell dator.

### [Set-AzVMADDomainExtension](Set-AzVMADDomainExtension.md)
Lägger till ett AD-domännamn till en virtuell dator.

### [Set-AzVMAEMExtension](Set-AzVMAEMExtension.md)
Aktiverar support för övervakning för SAP-system.

### [Set-AzVMBackupExtension](Set-AzVMBackupExtension.md)
Ställer in egenskaper för säkerhets kopiering på en virtuell dator.

### [Set-AzVMBginfoExtension](Set-AzVMBginfoExtension.md)
Lägger till BGInfo-tillägget på en virtuell dator.

### [Set-AzVMBootDiagnostic](Set-AzVMBootDiagnostic.md)
Ändrar startdiagnostik egenskaper för en virtuell dator.

### [Set-AzVMChefExtension](Set-AzVMChefExtension.md)
Lägger till en kock-anknytning till en virtuell dator.

### [Set-AzVMCustomScriptExtension](Set-AzVMCustomScriptExtension.md)
Lägger till ett anpassat skript tillägg till en virtuell dator.

### [Set-AzVMDataDisk](Set-AzVMDataDisk.md)
Ändrar egenskaper för en data disk för en virtuell dator.

### [Set-AzVMDiagnosticsExtension](Set-AzVMDiagnosticsExtension.md)
Konfigurerar Azure Diagnostics-tillägget på en virtuell dator.

### [Set-AzVMDiskEncryptionExtension](Set-AzVMDiskEncryptionExtension.md)
Aktiverar kryptering på en virtuell dator med IaaS i Azure.

### [Set-AzVMDscExtension](Set-AzVMDscExtension.md)
Konfigurerar DSC-tillägget på en virtuell dator.

### [Set-AzVMExtension](Set-AzVMExtension.md)
Uppdaterar tilläggs egenskaper eller lägger till ett tillägg till en virtuell dator.

### [Set-AzVMOperatingSystem](Set-AzVMOperatingSystem.md)
Anger egenskaper för operativ systemet för en virtuell dator.

### [Set-AzVMOSDisk](Set-AzVMOSDisk.md)
Anger operativ systemets disk egenskaper på en virtuell dator.

### [Set-AzVMPlan](Set-AzVMPlan.md)
Ställer in information om Marketplace på en virtuell dator.

### [Set-AzVMSourceImage](Set-AzVMSourceImage.md)
Anger bilden för en virtuell dator.

### [Set-AzVMSqlServerExtension](Set-AzVMSqlServerExtension.md)
Anger Azure SQL Server-tillägget på en virtuell dator.

### [Set-AzVmss](Set-AzVmss.md)
Ställer in specifika åtgärder på ett angivet VMSS.

### [Set-AzVmssBootDiagnostic](Set-AzVmssBootDiagnostic.md)
Anger den virtuella datorns skalnings profil för Autostart.

### [Set-AzVmssDiskEncryptionExtension](Set-AzVmssDiskEncryptionExtension.md)
Aktiverar disk kryptering på en virtuell dators skala.

### [Set-AzVmssOrchestrationServiceState](Set-AzVmssOrchestrationServiceState.md)
Anger tillstånd för Orchestration-tjänsten för VMSS.

### [Set-AzVmssOsProfile](Set-AzVmssOsProfile.md)
Anger profil egenskaperna för VMSS operativ system.

### [Set-AzVmssRollingUpgradePolicy](Set-AzVmssRollingUpgradePolicy.md)
Anger princip egenskaperna för VMSS rullande uppgradering.

### [Set-AzVmssStorageProfile](Set-AzVmssStorageProfile.md)
Anger lagrings profil egenskaper för VMSS.

### [Set-AzVmssVM](Set-AzVmssVM.md)
Ändrar tillstånd för en VMSS-instans.

### [Start-AzVM](Start-AzVM.md)
Startar en virtuell Azure-dator.

### [Start-AzVmss](Start-AzVmss.md)
Startar VMSS eller en uppsättning virtuella datorer i VMSS.

### [Start-AzVmssRollingOSUpgrade](Start-AzVmssRollingOSUpgrade.md)
Startar en rullande uppgradering för att flytta alla instanser av virtuell dators skalnings uppsättning till den senaste tillgängliga Platform image OS-versionen.

### [Stopp-AzVM](Stop-AzVM.md)
Stoppar en virtuell Azure-dator.

### [Stopp-AzVmss](Stop-AzVmss.md)
Stoppar VMSS eller en uppsättning virtuella datorer i VMSS.

### [Stopp-AzVmssRollingUpgrade](Stop-AzVmssRollingUpgrade.md)
Avbryter den befintliga rullande uppgraderingen för virtuell dator.

### [Test-AzVMAEMExtension](Test-AzVMAEMExtension.md)
Kontrollerar konfigurationen för AEM-tillägget.

### [Update-AzAvailabilitySet](Update-AzAvailabilitySet.md)
Uppdaterar en tillgänglighets uppsättning.

### [Update-AzContainerService](Update-AzContainerService.md)
Uppdaterar tillståndet för en behållar tjänst.

### [Update-AzDisk](Update-AzDisk.md)
Uppdaterar en disk.

### [Update-AzDiskEncryptionSet](Update-AzDiskEncryptionSet.md)
Uppdaterar en disk krypterings uppsättning.

### [Update-AzGallery](Update-AzGallery.md)
Uppdatera ett galleri.

### [Update-AzGalleryImageDefinition](Update-AzGalleryImageDefinition.md)
Uppdatera en bild definition för galleriet.

### [Update-AzGalleryImageVersion](Update-AzGalleryImageVersion.md)
Uppdatera en bild version i galleriet.

### [Update-AzImage](Update-AzImage.md)
Uppdaterar en bild.

### [Update-AzSnapshot](Update-AzSnapshot.md)
Uppdaterar en ögonblicks bild.

### [Update-AzVM](Update-AzVM.md)
Uppdaterar tillståndet för en virtuell Azure-dator.

### [Update-AzVmss](Update-AzVmss.md)
Uppdaterar tillståndet för en VMSS.

### [Update-AzVmssInstance](Update-AzVmssInstance.md)
Startar en manuell uppgradering av VMSS-instansen.

### [Update-AzVmssVM](Update-AzVmssVM.md)
Uppdaterar tillståndet för en VMSS VM.

