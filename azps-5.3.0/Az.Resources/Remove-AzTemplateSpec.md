...
## PROBLEMBESKRIVNING
Tar bort en specifik mall. Om parametern version **-version** anges tas bara den angivna versionen bort. Om ingen specifik version anges tas mallens specifikationer och alla dess versioner bort. Om flaggan **-Force** finns tillgänglig visas ingen uppmaning om att ta bort den.

## BESKRIVS

### Exempel 1: ta bort en specifik version med namn
```powershell
PS C:\> Remove-AzTemplateSpec -ResourceGroupName 'myRG' -Name 'MyTemplateSpec' -Version 'v1.0'
```

Tar bort version ' v 1.0 ' för mallnamnet "MyTemplateSpec" i resurs gruppen "myRG".

### Exempel 2: ta bort en specifik version via resurs-ID
```powershell
...
Tar bort version ' v 1.0 ' för mallnamnet med namnet "MyTemplateSpec" i resurs gruppen "myRG" för abonnemanget \{ subId \} .

### Exempel 3: ta bort en specifikation och alla versioner efter namn
```powershell
PS C:\> Remove-AzTemplateSpec -ResourceGroupName 'myRG' -Name 'MyTemplateSpec'
```

Tar bort den spec som heter ' MyTemplateSpec ' och alla dess versioner i resurs gruppen ' myRG '.

### Exempel 3: ta bort en specifikation och alla versioner efter resurs-ID
```powershell
...
Tar bort den spec som heter ' MyTemplateSpec ' och alla dess versioner i resurs gruppen ' myRG ' för abonnemanget \{ subId \} .
...
### -ResourceGroupName
Namnet på resurs gruppen för den här mallen.
...
### -Version
Den version av den mall som ska tas bort.
