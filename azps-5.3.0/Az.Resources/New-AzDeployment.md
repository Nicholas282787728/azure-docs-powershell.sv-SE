...
## PROBLEMBESKRIVNING
Cmdleten **New-AzDeployment** lägger till en distribution i den aktuella prenumerations omfattningen.
Detta inkluderar de resurser som krävs för distributionen.

En Azure-resurs är en användardefinierad Azure-enhet. En resurs kan finnas i en resurs grupp, till exempel databas server, databas, webbplats, virtuell dator eller lagrings konto.
Eller så kan det vara en prenumerations nivå resurs, till exempel roll definition, princip definition etc.

Om du vill lägga till resurser i en resurs grupp använder du den **nya-AzResourceGroupDeployment** som skapar en distribution i en resurs grupp.
Cmdleten **New-AzDeployment** skapar en distribution vid den aktuella prenumerationens omfattning, som distribuerar abonnemangs nivå resurser.

Om du vill lägga till en distribution vid abonnemang anger du platsen och en mall.
Platsen säger att Azure Resource Manager lagrar distributions data. Mallen är en JSON-sträng som innehåller enskilda resurser som ska distribueras.
...
Om du vill använda en anpassad mall för distributionen anger du parametern *TemplateFile* eller parametern *TemplateUri* .
...
Det här kommandot skapar en ny distribution vid det aktuella abonnemanget med hjälp av en anpassad mall och en mallfil på disk, med en definierad tagg-parameter.
...
### Exempel 2: använda ett anpassat mallfiler och en parameter fil för att skapa en distribution
```
...
Det här kommandot skapar en ny distribution vid det aktuella abonnemanget genom att använda en anpassad mall och en mallfil på en disk som har konverterats till en i minnet.
De två första kommandona läser texten för mallfilen på hård disken och konverterar den till en hash i minnet.
Det sista kommandot använder parametern *TemplateObject* för att ange den här hash-tabellen och parametern *TemplateParameterFile* för att ange en fil som innehåller parametrar och parameter värden.
Parametern *TemplateVersion* används för att ange versionen för mallen.
...
Loggnings nivå för distributions fel.
...
Hoppar över den dynamiska PowerShell-parametern för parametrar som kontrollerar om den tillgängliga mallen innehåller alla nödvändiga parametrar som används i mallen. Den här kontrollen uppmanar användaren att ange ett värde för de saknade parametrarna, men om du anger SkipTemplateParameterPrompt ignorerar den här uppmaningen och felvärderar omedelbart om en parameter inte är bunden i mallen. För icke-interaktiva skript kan-SkipTemplateParameterPrompt tillhandahållas för att ge ett bättre fel meddelande om inte alla nödvändiga parametrar uppfylls.
...
### -Tagg
Taggarna som ska läggas till i distributionen.
...
Kommaavgränsade resurs ändrings typer som ska uteslutas från What-If resultat. Tillämpbar när växeln-WhatIf eller-Confirm har ställts in.
