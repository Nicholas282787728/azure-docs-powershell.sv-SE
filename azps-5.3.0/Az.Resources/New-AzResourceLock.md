...
## PROBLEMBESKRIVNING
Cmdleten **New-AzResourceLock** skapar ett resurs lås.

## BESKRIVS

### Exempel 1: skapa ett resurs lås på en webbplats
```
...
Det här kommandot skapar ett resurs lås på en webbplats.

### Exempel 2: skapa ett resurs lås på en databas
```
...
Det här kommandot skapar ett resurs lås på en Azure-databas.
...
### -LockLevel
Anger nivån för låset.
För närvarande är giltiga värden CanNotDelete, ReadOnly.
...
### -ResourceGroupName
Anger namnet på en resurs grupp som låset gäller för eller som innehåller den resurs grupp som låset gäller för.
...
Om du till exempel vill ange en databas använder du följande format: `/subscriptions/` ID för `/resourceGroups/` resurs grupp namn `/providers/Microsoft.Sql/servers/` Server namn `/databases/` databas namn för att ange en resurs grupp, Använd följande format: `/subscriptions/` `/resourceGroups/` resurs GRUPPS namn för prenumerations-ID
...
### Microsoft. Azure. commands. ResourceManager. cmdlets. entities. LockLevel
