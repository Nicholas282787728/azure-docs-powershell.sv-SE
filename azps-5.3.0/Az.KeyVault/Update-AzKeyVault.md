...
## PROBLEMBESKRIVNING
Denna cmdlet uppdaterar tillståndet för ett Azure Key-valv.

## BESKRIVS

### Exempel 1: Aktivera rensnings skydd
```powershell
PS C:\> Get-AzKeyVault -VaultName $keyVaultName -ResourceGroupName $resourceGroupName | Update-AzKeyVault -EnablePurgeProtection
```

Aktiverar rensa skydd med rör-syntax.

### Exempel 2: Aktivera RBAC-auktorisering
```powershell
...
Aktiverar RBAC-auktorisering med rör-syntax.

### Exempel 3: Ange Taggar
```powershell
PS C:\> Get-AzKeyVault -VaultName $keyVaultName | Update-AzKeyVault -Tags @{key = "value"}
```

Anger taggarna för ett nyckelord som heter $keyVaultName.

### Exempel 4: Rensa Taggar
```powershell
PS C:\> Get-AzKeyVault -VaultName $keyVaultName | Update-AzKeyVault -Tags @{}
```

Tar bort alla Taggar för ett nyckelord som heter $keyVaultName.
...
Aktivera funktionerna för rensnings skydd för det här nyckelvärdet.
När den är aktive rad kan den inte avaktiveras.
Borttagning krävs för att den ska vara aktive rad.
...
Aktivera eller inaktivera det här nyckelvärdet för att tillåta data åtgärder via RBAC (rollbaserad åtkomst kontroll).
