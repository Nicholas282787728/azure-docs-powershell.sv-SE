...
## PROBLEMBESKRIVNING
Cmdleten **Add-AzVirtualNetworkSubnetConfig** lägger till en under nätverks konfiguration i ett befintligt Azure Virtual Network.
...
```
  ...
  I det här exemplet skapas en resurs grupp först som en behållare för de resurser som ska skapas. Därefter skapas en under nätverks konfiguration och används för att skapa ett virtuellt nätverk. Add-AzVirtualNetworkSubnetConfig används sedan för att lägga till ett undernät i det virtuella nätverk i minnet. Kommandot Set-AzVirtualNetwork uppdaterar det befintliga virtuella nätverket med det nya under nätet.

### Exempel 2: lägga till en delegering till ett undernät som läggs till i ett befintligt virtuellt nätverk
```powershell
...
I det här exemplet får du först ett befintligt virtuellt nätverk.
Därefter skapas ett Delegerings objekt i minnet.
Slutligen skapas ett nytt undernät med den delegering som läggs till i VNet. Den ändrade konfigurationen skickas till servern.
