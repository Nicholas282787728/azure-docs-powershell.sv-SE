...
### Exempel 1: skapa en PSRoutingRuleObject för skapande av en lucka med en vidarebefordrings regel
```powershell
...
### Exempel 2: skapa en PSRoutingRuleObject för skapande av en lucka med en regler för omdirigering
```powershell
...
### -CustomFragment
Fragment att lägga till i omdirigerings-URL: en. Fragment är den del av URL-adressen som kommer efter #. Ta inte med #.
...
### -CustomHost
Värd att omdirigera. Lämna tomt om du vill använda den inkommande värden som mål värd.
...
### -CustomPath
Den fullständiga sökvägen till omdirigeringen. Sökvägen får inte vara tom och måste börja med/. Lämna tomt om du vill använda den inkommande sökvägen som mål Sök väg.
...
### -CustomQueryString
Den uppsättning frågesträngar som ska placeras i URL-adressen för omdirigering. Om du anger det här värdet ersätts en befintlig frågesträng. lämna tomt om du vill behålla inkommande frågesträng. Frågesträngen måste vara i <key>=<value> format. Den första? & läggs till automatiskt, så ta inte med dem på fram sidan, men dela upp flera frågesträngar med &.
...
### -RedirectProtocol
Protokollet för mål platsen där trafiken omdirigeras. Standardvärdet är MatchRequest
...
### -RedirectType
Omdirigerings typ regeln används vid omdirigering av trafik. Standardvärdet flyttas
...
### -RulesEngineName
En referens till en specifik regel motor konfiguration som ska användas för den här vägen.
