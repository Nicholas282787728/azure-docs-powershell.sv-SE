...
## PROBLEMBESKRIVNING
Cmdleten **set-AzFrontDoor** uppdaterar en belastningsutjämnare för Last luckan. Om indataparametrarna inte anges används gamla parametrar från den befintliga främre dörren.

## BESKRIVS

### Exempel 1: uppdatera en befintlig främre dörr med FrontDoorName och ResourceGroupName.
```powershell
...
### Exempel 2: uppdatera en befintlig främre dörr med PSFrontDoor-objekt.
```powershell
...
### Exempel 3: uppdatera en befintlig främre dörr med ResourceId
```powershell
...
### Exempel 4: uppdatera BackendPoolSetting egenskapen EnforceCertificateNameCheck för en befintlig främre dörr med-DisableCertificateNameCheck switch parameter
Den främre dörren kan identifieras med FrontoorName och ResourceGroupName, PSFrontDoor-objekt eller ResourceId. (Se tre exempel till exempel) I exemplet nedan används objektet PSFrontDoor.
...
Om du vill inaktivera certifikat namns kontroll för HTTPS-begäranden till alla backend-pooler. Ingen effekt på icke-HTTPS-begäranden.
...
### -EnabledState
Drift status för belastnings utjämning för Last luckan.
...
### -InputObject
Objekt för front dörr att uppdatera.
...
### -Namn
Namnet på den främre dörren som ska uppdateras.
...
### -ResourceId
Resurs-ID för den främre dörren som ska uppdateras
