...
## Sammanfattning
Hämtar batchattribut under det angivna batch-kontot.
...
## PROBLEMBESKRIVNING
Cmdleten **Get-AzBatchPool** hämtar Azure Batch-pooler under batch-kontot som anges med parametern *BatchContext* .
Du kan använda *ID-* parametern för att få en enda pool, eller så kan du använda parametern *filter* om du vill hämta de pooler som matchar ett open data Protocol (OData)-filter.

## BESKRIVS

### Exempel 1: Hämta en pool utifrån ID
```
...
Med det här kommandot hämtas poolen med ID-nummer.

### Exempel 2: Hämta alla pooler med ett OData-filter
```
...
Det här kommandot får de pooler vars ID: n börjar med att använda parametern *filter* .
...
### -Filter
Anger den OData filter-sats som ska användas för att fråga efter pooler.
Om du inte anger något filter returneras alla pooler under batch-konto som anges med parametern *BatchContext* .
...
### -ID
Anger ID för den pool som ska visas.
