...
### Exempel 1: uppdatera ett befintligt kluster med namn
```powershell
...
Kommandot ovan uppdaterar SKU för Kusto-klustret "testnewkustocluster" i resurs gruppen "testrg".

### Exempel 2: uppdatera ett befintligt kluster med namn
```powershell
...
Kommandot ovan uppdaterar klustret "testnewkustocluster" i resurs gruppen "testrg" med en hanterad kund.
...
Ett booleskt värde som anger om klustrets diskar är krypterade.
...
Ett booleskt värde som anger om Double Encryption är aktiverat.
...
### -EnablePurge
Ett booleskt värde som anger om rensningen är aktive rad.
...
Ett booleskt värde som anger om streaming-funktionen är aktive rad.
...
Typen ' SystemAssigned, UserAssigned ' inkluderar både en implicit skapad identitet och en uppsättning användardefinierade identiteter.
...
Listan över användar identiteter som är kopplade till Kusto-klustret.
Nyckelorden för användar identitets ord lista är ARM resurs-ID i formatet: '/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.ManagedIdentity/userAssignedIdentities/{identityName} '.
...
Den användare tilldelade identiteten (ARM Resource-ID) som har åtkomst till den.
...
Listan över språk tillägg.
För att konstruera kan du läsa avsnittet anteckningar för LANGUAGEEXTENSIONVALUE-egenskaper och skapa en hash-tabell.
...
Ett booleskt värde som anger om den optimerade autoskalningsfunktionen är aktive rad eller inte.
...
Versionen av den definierade mallen, till exempel 1.
...
Klustrets externa klient organisationer.
För att konstruera kan du läsa avsnittet anteckningar för TRUSTEDEXTERNALTENANT-egenskaper och skapa en hash-tabell.
...
Resurs-ID för offentlig IP-adress för data hantering.
...
Motor tjänstens resurs-ID för offentlig IP-adress.
...
LANGUAGEEXTENSIONVALUE <ILanguageExtension [] >: listan med språk tillägg.
  - `[Name <LanguageExtensionName?>]`: Namnet på språk tillägget.

TRUSTEDEXTERNALTENANT <ITrustedExternalTenant [] >: klustrets externa klient organisationer.
  - `[Value <String>]`: GUID som representerar en extern klient organisation.
