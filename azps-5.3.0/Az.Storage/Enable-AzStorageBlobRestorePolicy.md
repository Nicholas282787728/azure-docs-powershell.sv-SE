...
## PROBLEMBESKRIVNING
Cmdleten **Enable-AzStorageBlobRestorePolicy** aktiverar BLOB Restore policy för Azure Storage Blob-tjänsten.

## BESKRIVS

### Exempel 1: aktiverar BLOB Restore policy för Azure Storage Blob-tjänsten på ett lagrings konto
```powershell
...
Detta kommando aktiverar först BLOB softdelete och changefeed, sedan aktiverar BLOB Restore policy och slutligen kontrollerar inställningen i egenskaper för Blob-tjänsten.
RestorePolicy för Blob-tjänsten måste vara mindre än DeleteRetentionPolicy. dagar.
BLOB softdelete och ChangeFeed måste aktive ras före aktivering av BLOB Restore policy.
Om softdelete och Changefeed just är aktiverade kan det vara nödvändigt att vänta en stund för att servern ska hantera inställningen innan den aktive ras.
...
### -RestoreDays
Anger antalet dagar som blobben kan återställas.
