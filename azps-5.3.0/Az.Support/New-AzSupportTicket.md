...
### Exempel 15: skapa en kvot support för att öka kvoten för en hanterad Azure SQL-instans. Använd Get-AzSupportService och Get-AzSupportProblemClassification för att hämta rätt GUID för SQL-hanterade instans tjänst problem klassificering.
```powershell
...
### Exempel 16: skapa ett support ärende genom att ange enskilda parametrar för kund kontakt i stället för CustomerContactDetail-objekt. 
```powershell
...
### Exempel 17: skapa ett support ärende med begäran om 24 x 7 svar från Azure. 
```powershell
