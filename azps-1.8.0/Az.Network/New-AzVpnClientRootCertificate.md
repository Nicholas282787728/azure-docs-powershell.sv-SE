...
Du kan hämta PublicCertData med hjälp av Windows PowerShell-kommandon som liknar detta: $Text = Get-Content-Path "C:\Azure\Certificates\ExportedCertficate.cer" $CertificateText = för ($i = 1; $i-lt $Text. length-1; $i + +) {$Text \[ $i \] }
