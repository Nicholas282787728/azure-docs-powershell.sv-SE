...
## PROBLEMBESKRIVNING
**New-AzApiManagementOperation-** cmdleten skapa en API-åtgärd.

## BESKRIVS

### Exempel 1: skapa en API-hanterings åtgärd
```powershell
...
Det här kommandot skapar en API-hanterings åtgärd.

### Exempel 2: skapa en API-hanterings åtgärd med förfrågnings-och svars uppgifter
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
I det här exemplet skapas en API-hanterings åtgärd med förfrågnings-och svars uppgifter.

## MALLPARAMETRAR

### -ApiId
Anger ID för API-hanterings åtgärden.
...
### -ApiRevision
Identifierare för API-revision. Denna parameter är valfri. Om du inte anger den kopplas åtgärden till den för tillfället aktiva API-ändringen.
...
### -Beskrivning
Anger beskrivningen av den nya API-åtgärden.
...
### -Metod
Anger HTTP-metoden för den nya API-hanterings åtgärden.
...
### -Namn
Anger visnings namnet på den nya API-hanterings åtgärden.
...
### -Begäran
Anger information om API-hanterings åtgärden.
...
### -Svar
Anger en matris med möjliga API-hanterings svar.
...
### -TemplateParameters
Anger en matris med parametrar som definierats i parametern *UrlTemplate*.
Om du inte anger den här parametern genereras ett standardvärde baserat på *UrlTemplate*.
