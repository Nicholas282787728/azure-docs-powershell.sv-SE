...
## PROBLEMBESKRIVNING
Cmdleten **set-AzApiManagementPolicy** anger den angivna SCOPEALTERNATIV för API-hantering.
...
### -ApiRevision
Identifierare för API-revision. Denna parameter är valfri. Om det inte anges uppdateras policyn i den för tillfället aktiva API-ändringen.
...
### -Format
Anger principens format. Om `application/vnd.ms-az-apim.policy+xml` du använder, måste uttryck i principen vara XML-avmarkerat. När du använder `application/vnd.ms-az-apim.policy.raw+xml` den är det **inte** nödvändigt att principen är avmarkerad.
Standardvärdet är `application/vnd.ms-az-apim.policy+xml` .
...
### -PolicyUrl
URL-adressen där princip dokumentet finns. Den här parametern är obligatorisk om-princip-eller-PolicyFilePath inte har angetts.
