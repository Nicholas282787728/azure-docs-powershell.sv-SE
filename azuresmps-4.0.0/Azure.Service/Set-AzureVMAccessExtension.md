...
## Sammanfattning
Anger VMAccess-tillägget för en virtuell dator.
...
## PROBLEMBESKRIVNING
Cmdleten **set-AzureVMAccessExtension** lägger till VMAccess-tillägget för virtuell dator till en virtuell dator. VMAccess-tillägget kan användas för att ange ett tillfälligt lösen ord och det bör ändras direkt efter att du har loggat in på datorn.

## BESKRIVS

### Exempel 1: Ange tillägget VMAccess som används för en viss virtuell dator
```
PS C:\> Set-AzureVMAccessExtension -VM $VM -UserName $User -Password $PWD;
```

Det här kommandot anger det VMAccess-tillägg som används för den angivna virtuella datorn enligt variabel $VM.

## MALLPARAMETRAR

### -Inaktivera
Anger att den här cmdleten ställer in det tilläggs tillstånd som ska inaktive ras.
...
### -Lösen ord
Anger lösen ordet för att återställa den virtuella datorns autentiseringsuppgifter.
...
### -ReferenceName
Anger referens namnet för Access-tillägget.
...
För kommande uppdateringar ska du ange det tidigare använda referens namnet när du uppdaterar tillägget.
...
### -Avinstallera
Anger om den här cmdleten avinstallerar åtkomst tillägget.
...
### -UserName
Anger det användar namn som används i denna cmdlet för att återställa den virtuella datorns autentiseringsuppgifter.
