...
## Sammanfattning
Lägger till en post för en BLOB-lagringsenhet i en HDInsight-konfiguration.
...
Cmdleten **Add-AzureHDInsightStorage** lägger till en post för BLOB-lagringsenhet i en Azure HDInsight-konfiguration.

## BESKRIVS

### Exempel 1: lägga till ett lagrings konto
```
PS C:\>$StoreConfig = Add-AzureHDInsightStorage -Config $Config -StorageAccountName "MyStorage" -StorageAccountKey "Key"
```

Det här kommandot lägger till ett lagrings konto med namnet min lagring i det konfigurationsfiler som lagras i $Config och lagrar sedan konfigurationen i $StoreConfig-variabeln.

### Exempel 2: Konfigurera flera lagrings konton
```
...
De fjärde, femte och sjätte kommandona hämtar autentiseringsuppgifter för det aktuella abonnemanget och för Oozie och Hive och lagrar sedan autentiseringsuppgifterna i variabler.
...
- **Set-AzureHDInsightDefaultStorage** om du vill ange standard lagrings kontot för konfigurationen till MyBlobStorage.blob.Core.Windows.net
- **Add-AzureHDInsightStorage** om du vill lägga till ett andra lagrings konto med MySecondBlobStorage.blob.Core.Windows.net i konfigurationen
- **Add-AzureHDInsightStorage** om du vill lägga till en metastore för Oozie och en metastore för struktur i konfigurationen
...
Denna cmdlet lägger till information om lagrings konto i det objekt som den här parametern anger.
...
### -StorageAccountKey
Anger den lagrings konto plats som används för att komma åt ett lagrings konto.
