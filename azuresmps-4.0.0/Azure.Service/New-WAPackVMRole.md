...
Cmdleten **New-WAPackVMRole** skapar en roll för virtuell dator.

## BESKRIVS

### Exempel 1: skapa en virtuell dator roll (emulering av WAP)
```
PS C:\> New-WAPackVMRole -Name "ContosoVMRole01" -Label "ContosoVMRoleLabel01" -ResourceDefinition $resdef
```

Eftersom vi inte anger någon moln tjänst (emulering med WAP) skapar kommandot en moln tjänst åt oss som har samma namn som den virtuella dator rollen.
I det här fallet skapar följande kommando rollen virtuell dator med namnet ContosoVMRole01, Label ContosoVMRoleLabel01.
Observera att den resurs definition som används här måste skapas manuellt med PowerShell.

## MALLPARAMETRAR

### -CloudService
Anger en moln tjänst för rollen virtuell dator.
...
### -Etikett
Anger en etikett för den virtuella dator rollen.
...
### -Namn
Anger ett namn för den virtuella dator rollen.
...
### -ResourceDefinition
Anger en resurs definition för rollen virtuell dator.
