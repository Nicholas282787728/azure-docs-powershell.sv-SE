...
## Sammanfattning
Exporterar en tillstånds status till en fil.
...
## PROBLEMBESKRIVNING
Cmdleten **export-AzureVM** exporterar statusen för en virtuell dator till en XML-fil.
...
### Exempel 1: exportera en virtuell dator
```
PS C:\> Export-AzureVM -ServiceName "ContosoService" -Name "ContosoRole06" -Path "C:\vms\VMstate.xml"
```

Det här kommandot exporterar statusen för den virtuella datorn till VMstate.xml-filen.
...
### -Namn
Anger namnet på den virtuella dator för vilken denna cmdlet exporterar status.
...
### -Path
Anger sökvägen och fil namnet som denna cmdlet sparar den virtuella datorns tillstånd i.
...
### -ServiceName
Anger namnet på den Azure-tjänst som är värd för den virtuella datorn.
