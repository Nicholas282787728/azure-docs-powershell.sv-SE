...
## PROBLEMBESKRIVNING
Cmdleten **Update-AzNetAppFilesAccount** ändrar en ANF Active Directory-konfiguration.
...
Det här kommandot utför en uppdatering av angiven Active Directory-konfiguration och ändrar det användar namn som tillhandahålls.
...
### -AccountObject
Kontot för Active Directory-objektet
...
### -AdName
Namn på Active Directory-datorn.
Denna valfria parameter används endast när du skapar Kerberos-volymen
...
### -BackupOperator
Användare som ska läggas till i den inbyggda ansvarig för säkerhets kopiering Active Directory.
En lista med unika användar namn utan domän specifikation
...
### -DNS
Kommaavgränsad lista med IP-adresser för DNS-servrar (endast IPv4) för Active Directory-domänen
...
### -InputObject
Active Directory-objekt som ska tas bort
...
### -KdcIP
IP-adresser för KDC-servrar för Active Directory-datorn.
Denna valfria parameter används bara när du skapar Kerberos-volymen.
...
### -Lösen ord
Vanligt text lösen ord för Active Directory Domain Administrator är värdet maskerat i svaret
...
Om LDAP via SSL/TLS är aktiverat måste LDAP-klienten ha Base64-kodat Active Directory-certifikat tjänstens självsignerade rot certifikat UTFÄRDAre, denna valfria parameter används endast för Dual Protocol med LDAP-mappning av volymer.
...
### -Site
Den Active Directory-plats som tjänsten begränsar identifiering av domänkontrollant till
...
Det här namnet registreras som ett dator konto i ANNONSen och används för montering av volymer
...
### -Username
Användar namn för Active Directory-domän administratör
