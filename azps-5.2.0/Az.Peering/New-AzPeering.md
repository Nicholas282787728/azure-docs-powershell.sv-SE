...
Skapa en ny direkt peering med en enda anslutning vid Malmö med PeerAsn 65000

### Skapa ny Exchange-peering
```powershell
#Gets the ASN
PS C:> $asn = Get-AzPeerAsn -PeerName Contoso
...
Skapa en ny Exchange-peering

### Konvertera bakåtkompatibel peering till ARM-peering
```powershell
#Gets the ASN
PS C:> $asn = Get-AzPeerAsn -PeerName Contoso
...
### -DirectConnection
Skapa en ny direkt förbindelse med New-AzExchangePeeringConnection och pipe till det här kommandot.
...
### -ExchangeConnection
Skapa en ny Exchange-anslutning med New-AzExchangePeeringConnection och pipe till det här kommandot.
...
### -InputObject
Använd Get-AzLegacyPeering för att hämta det här objektet.
...
### -MicrosoftNetwork
Välj det Microsoft-nätverk du vill använda.
...
### -PeerAsnResourceId
Resurs-ID för peer-PNRP. Använd Get-AzPeerAsn för att hämta ID.
...
### -PeeringLocation
Den fysiska platsen skiljer sig från Azure-regionen.
Använd \<kind\> Orts namn Get-AzPeeringLocation-as-tangenten.
...
### -SKU
Välj Basic_Direct_Free eller Premium_Direct_Free såvida det inte uttryckligen anges att välja ett annat alternativ.
...
### -Tagg
De taggar som ska kopplas till Microsofts peering service.
