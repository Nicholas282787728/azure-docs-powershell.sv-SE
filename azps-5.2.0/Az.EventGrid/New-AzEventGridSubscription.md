...
ID eller URI för Azure Active Directory-programmet (AAD) för att hämta åtkomst-token som ska inkluderas som Bearer-token i leverans förfrågningar. Gäller bara webhook som mål.
...
Klient-ID: t för Azure Active Directory (AAD) för att få åtkomsttoken som kommer att inkluderas som Bearer-token i leverans förfrågningar. Gäller bara webhook som mål.
...
### -DeliverySchema
Schemat som ska användas när du levererar händelser till målet. Möjliga värden är: eventgridschema, CustomInputSchema eller cloudeventv01schema. Standardvärdet är CustomInputSchema.
...
### -MaxEventsPerBatch
Maximalt antal händelser i en grupp. Värdet måste vara mellan 1 och 5000. Den här parametern är giltig när Endpint-typen endast är webhook.
...
Önskad batchstorlek i kilobyte. Värdet måste vara mellan 1 och 1024. Den här parametern är giltig när Endpint-typen endast är webhook.
