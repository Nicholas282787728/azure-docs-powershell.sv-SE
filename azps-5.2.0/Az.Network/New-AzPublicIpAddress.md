...
### Exempel 4: skapa en ny offentlig IP-adress utifrån ett prefix
```powershell
$publicIp = New-AzPublicIpAddress -Name $publicIpName -ResourceGroupName $rgName -AllocationMethod Static -DomainNameLabel $dnsPrefix -Location $location
...
Det här kommandot skapar en ny global offentlig IP-adressresurs. En DNS-post skapas för $dnsPrefix. $location. cloudapp.azure.com som pekar på den offentliga IP-adressen för den här resursen. En global offentlig IP-adress tilldelas omedelbart till den här resursen.
Det här alternativet stöds endast för "standard" SKU och "static"-AllocationMethod.
