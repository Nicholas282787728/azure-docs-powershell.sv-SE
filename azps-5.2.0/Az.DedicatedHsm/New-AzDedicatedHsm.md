...
## Sammanfattning
Skapa eller uppdatera en dedikerad HSM i det angivna abonnemanget.
...
### Exempel 1: skapa en dedikerad HSM i ett befintligt virtuellt nätverk
```powershell
...
Det här kommandot skapar en dedikerad HSM till ett befintligt virtuellt nätverk.

**Obs!** För närvarande är `New-AzDedicatedHsm` det en begränsning att den returneras innan HSM är fullt etablerad på Azure.
När du har skapat en ny HSM ber vi dig be om dess status `Get-AzDedicatedHsm` och kontrol lera att den `Provisioning State` är `Succeeded` före användning.
...
### -Plats
Den Azure-plats där dedikerad HSM ska skapas.
...
### -NetworkInterface
Anger listan med resurs-ID: n för de nätverks gränssnitt som är associerade med den dedikerade HSM.
För att konstruera kan du läsa avsnittet anteckningar för NETWORKINTERFACE-egenskaper och skapa en hash-tabell.
...
### -ResourceGroupName
Namnet på resurs gruppen som resursen tillhör.
...
### -StampId
Det här fältet används när RP inte stöder zoner för tillgänglighet.
...
NETWORKINTERFACE <INetworkInterface [] >: anger listan med resurs-ID för de nätverks gränssnitt som är kopplade till den dedikerade HSM.
