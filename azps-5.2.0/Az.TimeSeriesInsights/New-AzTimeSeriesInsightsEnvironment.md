...
### Exempel 1: skapa en gen1 för tids serier
```powershell
...
Det här kommandot skapar en gen1 tids serie miljö.

### Exempel 2: skapa en Gen2 för tids serier
```powershell
...
Det här kommandot skapar en Gen2 tids serie miljö.
...
### -Kapacitet
Kapaciteten för SKU: n.
För gen1-miljöer kan det här värdet ändras för att stöd skalas utanför miljöer efter att de har skapats.
...
Listan med händelse egenskaper som används för att partitionera data i miljön.
För att konstruera kan du läsa avsnittet anteckningar för PARTITIONKEYPROPERTY-egenskaper och skapa en hash-tabell.
...
### -StorageAccountKey
Värdet för hanterings knappen som beviljar tjänsten tids serie insikter skriv åtkomst till lagrings kontot.
...
### -StorageAccountName
Namnet på det lagrings konto som ska innehålla miljöns långsiktiga data.
...
Beteendet för tids serie insikter bör tas när miljöns kapacitet har överskridits
...
### -Tagg
Nyckeltal för fler egenskaper för resursen.
...
Listan med händelse egenskaper som används för att definiera miljöens tidsserie-ID. För att konstruera kan du läsa avsnittet anteckningar för TIMESERIESIDPROPERTY-egenskaper och skapa en hash-tabell.
...
ISO8601 TimeSpan som anger hur många dagar miljöns händelser ska vara tillgängliga för frågor från det varmt Store.
...
### Microsoft. Azure. PowerShell. cmdletar. TimeSeriesInsights. Models. Api20200515. IEnvironmentResource
...
TIMESERIESIDPROPERTY <ITimeSeriesIdProperty [] >: listan med händelse egenskaper som används för att ange miljöens tidsserie-ID.
  - `[Name <String>]`: Namnet på egenskapen.
  - `[Type <PropertyType?>]`: Typen av egenskap.
