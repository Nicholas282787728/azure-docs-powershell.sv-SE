...
Skapar ett certifikat policy objekt för minnet. (automatiskt genererat)
...
### Exempel 3: skapa ett certifikat för ett alternativt subjekt namn (eller ett SAN)
...
### -DnsName
Anger DNS-namnen i certifikatet. Alternativa namn för subjekt (San) kan anges som DNS-namn.
...
> [!NOTE]
> Om du måste använda ett kommatecken (,) eller en punkt (.) i en egenskap i `SubjectName` parametern måste du omge egenskaps fältet med citat tecken. Du kan till exempel använda O = "contoso, Ltd." i fältet organisations namn.
