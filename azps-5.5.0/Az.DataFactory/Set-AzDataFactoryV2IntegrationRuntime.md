...
## BESKRIVNING
Med Set-AzDataFactoryV2IntegrationRuntime-cmdleten uppdateras en integrationskörning med specifika parametrar.

## EXEMPEL

### Exempel 1: Uppdatering av körningskörning för integrering.
```
...
### Exempel 2: Dela själv värdbaserade integrationer runtime.
```
PS C:\> Set-AzDataFactoryV2IntegrationRuntime -ResourceGroupName 'rg-test-dfv2' -DataFactoryName 'test-df-eu2' -Name 'test-selfhost-ir' `
...
Cmdleten lägger till ADF för att använda körningen för delad integrering. När du `-SharedIntegrationRuntimeResourceId` använder parameter måste även `-Type` parametern inkluderas. Observera att data factory måste tilldelas behörighet att använda integrationskörningen innan du kör cmdleten.

### Exempel 3: Konfigurera Self-Hosted IR som proxy för Azure-SSIS IR i ADF.
```
...
Cmdleten uppdaterar Azure-SSIS-integrering runtime för att använda self-hosted integration runtime som dataproxy.
...
### -DataProxyStagingPath
Den sökväg i mellanlagringsdatalagringsplatsen som ska användas när data flyttas mellan Self-Hosted och Azure-SSIS Integration Runtimes används en standardbehållare om det inte är ansenliga
...
### -Edition
Versionen för SSIS integration runtime, som kan vara Standard eller Enterprise, är standard standard om det inte anges.
...
### -LicenseType
Den licenstyp som du vill välja för SSIS IR. Det finns två typer: LicenseIncluded eller BasePrice. Om du är kvalificerad för AHUB-priser (Azure Hybrid Use Benefit) väljer du Baspris. Om inte, väljer du LicenseIncluded.
...
### -VNetId
ID för det VNet som integreringen runtime ansluter till.
