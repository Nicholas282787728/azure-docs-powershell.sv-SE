...
ms.locfileid: "100222551"
---
# Remove-AzEventGridSubscription

## SYNOPSIS
Tar bort en Azure Event Grid-händelseprenumeration.
...
## BESKRIVNING
Tar bort en Azure Event Grid-händelseprenumeration för ett Azure Event Grid-avsnitt, en resurs, en Azure-prenumeration eller resursgrupp.
...
Tar bort \` händelseprenumerationen EventSubscription1 till \` ett Azure Event \` Grid-ämne ämne1 i \` \` resursgruppen MyResourceGroupName. \`
...
Tar bort \` händelseprenumerationen EventSubscription1 \` till en \` resursgrupp MyResourceGroupName. \`
...
Tar bort händelseprenumerationen \` EventSubscription1 \` till azure-standardprenumerationen.
...
Tar bort händelseprenumerationen \` EventSubscription1 \` till ett namnområde i Händelsehubben.
...
Tar bort händelseprenumerationen \` EventSubscription1 \` till ett händelserutnätsämne.
...
Namnet på händelseprenumerationen som måste tas bort.
...
### -PassThru
Returnerar status för åtgärden Ta bort. Som standard genererar inte den här cmdleten några utdata.
...
### -ResourceId
Identifierare för resursen vars händelseprenumeration måste tas bort.
