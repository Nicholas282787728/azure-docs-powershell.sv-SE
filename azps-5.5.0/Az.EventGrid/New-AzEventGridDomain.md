...
## BESKRIVNING
Skapar en ny Azure Event Grid-domän. När domänen har skapats kan ett program publicera händelser till ämnesslutpunkten.

## EXEMPEL

### Exempel 1

Skapar en domändomän för händelserutnät1 i den \` angivna geografiska platsen \` \` westus2 \` i \` resursgruppen MyResourceGroupName. \`
...
Skapar en domändomän för händelserutnät1 i den angivna geografiska platsen \` \` \` westus2 \` i \` resursgruppen MyResourceGroupName med angivna \` taggar"Avdelning" och "Miljö".
...
### -PublicNetworkAccess
Det här anger om trafik tillåts i offentliga nätverk. Den är aktiverad som standard. Du kan begränsa ytterligare till specifika IP-adresser genom att konfigurera parametrar för InboundIpRule. Tillåtna värden inaktiveras och aktiveras.
...
### -Tag
Hashtabell som representerar resurstaggar.
