...
ms.locfileid: "100220030"
---
# Get-AzEventGridDomain

## SYNOPSIS
Hämtar information om en domän i händelserutnätet eller får en lista över alla domäner i händelserutnätet i den aktuella Azure-prenumerationen.
...
## BESKRIVNING
Med Get-AzEventGridDomain-cmdleten får du antingen information om en angiven domän i händelserutnätet eller en lista över alla domäner i händelserutnätet i den aktuella Azure-prenumerationen.
Om domännamnet anges returneras information om en enda händelserutnätsdomän.
Om domännamnet inte anges returneras en lista med domäner. Antalet element som returneras i den här listan kontrolleras av den översta parametern. Om det översta värdet inte anges eller $null, innehåller listan alla de domänobjekt som returneras på en gång. I annat fall anges det högsta antalet element som ska returneras i listan.
Om fler domäner fortfarande är tillgängliga ska värdet i NextLink användas i nästa samtal för att få nästa sida med domäner.
...
Hämtar information om domändomänen Händelserutnät1 \` \` i \` resursgruppen MyResourceGroupName. \`
...
Hämtar information om domänen Händelserutnät \` domän1 \` i resursgruppen \` MyResourceGroupName \` med alternativet ResourceId.
...
Lista alla domäner i händelserutnätet i resursgruppen \` MyResourceGroupName \` utan sidnumrering (alla domäner returneras med en bild)
...
Lista de händelserutnätsdomäner (om det finns) i resursgruppen \` MyResourceGroupName som motsvarar \` $odataFilter 10-domäner i taget. Om det finns fler resultat $result. NextLink kommer inte att $null. För att få nästa sida(er) med domäner förväntas användaren ringa Get-AzEventGridDomain och använder resultat. NextLink från föregående samtal. Uppringaren bör sluta när resultatet är det. NextLink blir $null.
...
Lista alla händelserutnätsdomäner i Azure-prenumeration utan sidbrytning (alla domäner returneras med en bild)
...
Lista händelserutnätsdomänerna (om några) i Azure-prenumerationen som uppfyller $odataFilter 20 domäner i taget. Om det finns fler resultat $result. NextLink kommer inte att $null. För att få nästa sida(er) med domäner förväntas användaren ringa Get-AzEventGridDomain och använder resultat. NextLink från föregående samtal. Uppringaren bör sluta när resultatet är det. NextLink blir $null.
...
### -NextLink
Länken för nästa sida med resurser som ska erhållas.
Det här värdet inhämtas med det Get-AzEventGrid cmdlet-anropet när fler resurser fortfarande är tillgängliga för frågor.
...
### -ODataQuery
OData-frågan som används för att filtrera listresultaten.
Filtrering är för närvarande endast tillåtet för egenskapen Namn. Exempel på åtgärder som stöds: CONTAINS, eq (för lika med), ne (för inte lika med), AND, OR och NOT.
...
### -Top
Maximalt antal resurser som ska erhållas.
Giltigt värde är mellan 1 och 100.
Om det högsta värdet anges och fler resultat fortfarande är tillgängliga innehåller resultatet en länk till nästa sida som du vill fråga i NextLink.
Om det högsta värdet inte anges returneras den fullständiga listan med resurser på en gång.
