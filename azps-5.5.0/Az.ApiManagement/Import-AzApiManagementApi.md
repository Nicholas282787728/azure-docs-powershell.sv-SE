...
## BESKRIVNING
Cmdleten **Import-AzApiManagementApi** importerar ett Azure API Management API från en fil eller en URL-adress i URL-formatet WEB Application Description Language (SÅSL), WSDL (Web Services Description Language) eller swagger-formatet (Web Application Description Language).

## EXEMPEL

### Exempel 1: Importera ett API från en BINÄRL-fil
```powershell
...
Med det här kommandot importeras ett API från den angivna CYLINDERL-filen.
...
### Exempel 3: Importera ett API från en CYLINDERL-länk
```powershell
PS C:\>$ApiMgmtContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Med det här kommandot importeras ett API från den angivna CYLINDERL-länken.

### Exempel 4: Importera ett API från en Open Api-länk
```powershell
...
Det här kommandot importerar ett API från den angivna Open 3.0-specifikationslänken.

### Exempel 5: Importera ett API från en Open Api-länk till en ApiVersion-uppsättning
...
Med det här kommandot importeras ett API från det angivna 3.0-specifikationsdokumentet för Open 3.0 och skapar ett nytt ApiVersion.

## PARAMETERS

### -ApiId
Anger ett ID för det API som ska importeras.
...
### -ApiRevision
Identifierare för API-revidering. Den här parametern är valfri. Om inget anges görs importen till den aktuella aktiva ändringen eller ett nytt API.
...
### -ApiType
Den här parametern är valfri med standardvärdet Http. Alternativet Soap är endast tillämpligt när du importerar WSDL och skapar ett SOAP-genomgående API.
...
### -Path
Anger en webb-API-sökväg som den sista delen av API:ets offentliga URL.
...
### -Protocol
Web API-protokoll (http, https). Protokoll över vilka API görs tillgängliga. Den här parametern är valfri. Om den anges åsidosätter den protokollen som anges i specifikationerna.
...
### -ServiceUrl
En URL för webbtjänsten som visar API:t. Den här URL-adressen används endast av Azure API Management och görs inte offentlig. Den här parametern är valfri. Om den har angetts åsidosätter den ServiceUrl som anges i dokumentet för specifikationer.
...
### -SpecificationFormat
Anger specifikationens format.
psdx_paramvalues , Wsdl och Swagger.
...
### -SpecificationPath
Anger specifikationens filsökväg.
...
### -WsdlEndpointName
Lokalt namn på WSDL-slutpunkt (port) som ska importeras. Måste vara 1 till 400 tecken långt. Den här parametern är valfri och krävs endast för import av Wsdl. Standardvärdet är $null.
...
### -WsdlServiceName
Det lokala namnet på WSDL-tjänsten som ska importeras. Måste vara 1 till 400 tecken långt. Den här parametern är valfri och krävs endast för import av Wsdl. Standardvärdet är $null.
