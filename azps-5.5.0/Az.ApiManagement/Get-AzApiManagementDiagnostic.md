...
## BESKRIVNING
**Get-AzApiManagementDiagnostic** hämtar information om den diagnostik som konfigurerats i api-hanteringstjänsten i en viss omfattning.

## EXEMPEL

### Exempel 1: Hämta alla diagnostikverktyg som konfigurerats i klientorganisationens omfattning.
```powershell
...
Med det här kommandot får du all diagnostik som konfigurerats i tjänsten Api Management.

### Exempel 2: Hämta all diagnostik som konfigurerats i API-omfattningen
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Med det här kommandot får du all diagnostik som konfigurerats i `echo-api` API-omfattningen

### Exempel 3: Hämta API-omfattningsdiagnostik som anges av ett ID
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Det här kommandot får `applicationinsights` den diagnostik som konfigurerats i `echo-api` API.

## PARAMETERS

### -ApiId
Identifierare för befintligt API.
Om detta anges returneras API-omfattningsdiagnostik.
...
Om den anges returneras produktomfattningen.
Det här parametrarna är valfritt.
...
### -ResourceId
Arm Resource Identifier för ett diagnostik- eller API-diagnostik. Om detta anges försöker diagnostik söka efter diagnostik med identifieraren. Den här parametern är obligatorisk.
