...
## BESKRIVNING
Cmdleten **Set-AzVmssVM** ändrar statusen för en VMSS-instans (Virtual Machine Scale Set).

## EXEMPEL

### Exempel 1

Ändrar tillståndet för en VMSS-instans. (autogenererat)
...
### -InstanceId
Anger ID för VMSS-instansen för vilken den här cmdleten ändrar status.
...
### -PerformMaintenance
Anger att den här cmdleten utför underhåll på en virtuell dator i VMSS.
...
### -Redepdistribution
Anger att denna cmdlet distribuerar om en virtuell dator i VMSS.
...
### -Reimage
Anger att den här cmdleten tar fram VMSS-instansen på nya sätt.
...
### -ReimageAll
Anger att cmdleten tar fram alla diskar i VMSS-instansen.
...
### -ResourceGroupName
Anger namnet på resursgruppen som innehåller VMSS-instansen.
...
### -SimulateEviction
Anger att den här cmdleten simulerar hur den virtuella dekorfärgen för den virtuella datorn i en VM-skaluppsättning kommer att minska.
Det tar inte mer än 30 minuter innan du anropar API:t.
...
### -VMScaleSetName
Anger namnet på VMSS-instansen som den här cmdleten ändrar.
