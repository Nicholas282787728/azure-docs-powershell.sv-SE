...
## BESKRIVNING
Uppdaterar statusen för en Vmss VM.  För tillfället är den enda tillåtna uppdateringen att lägga till en hanterad datadisk.

## EXEMPEL

### Exempel 1: Lägga till en hanterad datadisk i en VMss VM med hjälp av New-AzVMDataDisk
```powershell
...
### Exempel 2: Lägga till en hanterad datadisk i en VMss VM med hjälp av Add-AzVMDataDisk
```powershell
PS C:\> $disk = Get-AzDisk -ResourceGroupName $rgname -DiskName $diskname0
PS C:\> $VmssVM = Get-AzVmssVM -ResourceGroupName "myrg" -VMScaleSetName "myvmss" -InstanceId 0
...
Uppdaterar statusen för en Vmss VM. (autogenererat)
...
### -ProtectFromScaleIn
Anger att skalningen för den virtuella datorns skala inte ska övervägas för borttagning vid en skalning.
...
Anger att modelluppdateringar eller -åtgärder (inklusive skalning) som initierats på VMSS inte ska tillämpas på den VMSS VM.
...
Skaluppsättning för lokal virtuell dator med VM-objekt
