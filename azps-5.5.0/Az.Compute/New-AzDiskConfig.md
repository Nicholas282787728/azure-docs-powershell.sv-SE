...
Med det första kommandot skapas ett lokalt tomt diskobjekt med en storlek på 5 GB Standard_LRS en lagringskontotyp. Den ställer också in Windows OS-typ och aktiverar krypteringsinställningar. Andra och tredje kommandona anger inställningarna för diskkrypteringsnyckeln och nyckelkrypteringsnyckeln för diskobjektet. Det senaste kommandot tar diskobjektet och skapar en disk med namnet 'Disk01' i resursgruppen 'ResourceGroup01'.
...
Med det första kommandot skapas ett lokalt diskobjekt för uppladdning.
Det andra kommandot tar diskobjektet och skapar en disk med namnet 'Disk01' i resursgruppen 'ResourceGroup01'.
Det tredje kommandot får SAS URL för disken.
Det fjärde kommandot får statusen för disken.
Om disktillståndet är ReadyToUpload kan en användare ladda upp en disk från blob-lagring till SAS Url-disk med AzCopy.
Under uppladdningen ändras disktillståndet till "ActiveUpload".
Det senaste kommandot återkallar diskåtkomsten för SAS URL.
...
Skapa en skiva från en bildversion av ett delat galleri.  ID är ID för bildversionen av det delade galleriet. Lun behövs bara om källan är en datadisk.
...
### -CreateOption
Anger om denna cmdlet skapar en disk i den virtuella datorn från en plattform eller användarbild, skapar en tom disk eller bifogar en befintlig disk.
...
### -DiskAccessId
Hämtar eller anger ARM-ID för DiskAccess-resursen för att använda privata slutpunkter på.
...
### -DiskMBpsReadOnly
"beskrivning": "The total throughput (MBps) that will be allowed across all VMs montering the shared disk as ReadOnly. MBps innebär miljontals byte per sekund – MB här används ISO-notationen, som är 10 stor.
...
Objektet GalleryImageReference.  Krävs om du skapar en galleribild. Id:t blir det ARM ID för den delade galleybildversionen som du skapar en skiva från.
En lun behövs om kopians källa är en av datadiskarna i galleribilden. Om null kopieras OS-hårddisken för bilden.
...
### -HyperVGeneration
Hypervisor-generationen av den virtuella datorn. Gäller endast OS-diskar.  Tillåtna värden är V1 och V2.
...
### -ImageReference
Anger bildreferensen på en skiva.
...
### -MaxSharesCount
Maximalt antal virtuella maskiner som kan anslutas till disken samtidigt.
Ett värde större än en anger en disk som kanmonteras på flera virtuella maskiner samtidigt.
...
### -NetworkAccessPolicy
Principen för nätverksåtkomst definierar principen för nätverksåtkomst.
...
### -UploadSizeInBytes
Anger storleken på innehållet i överföringen, inklusive VHD-sidfoten när CreateOption är Uppladdning.  Det här värdet bör vara mellan 20972032 (20 MiB + 512 byte för VHD-sidfoten) och 35183298347520 byte (32 TiB + 512 byte för VHD-sidfoten).
...
### -Zone
Anger den logiska zonlistan för Disk.
