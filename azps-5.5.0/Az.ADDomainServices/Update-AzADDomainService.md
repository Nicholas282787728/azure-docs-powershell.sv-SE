...
### Exempel 1: Uppdatera AzADDomainService efter ResourceGroupName och namn
```powershell
...
Uppdatera AzADDomainService efter ResourceGroupName och namn

### Exempel 2: Uppdatera AzADDomainService med Inputobject
```powershell
...
Uppdatera AzADDomainService genom Inputobject
...
En flagga för att avgöra om SyncKerberosPasswords ska vara aktiverat eller inaktiverat.
...
En flagga för att avgöra om SyncNtlmPasswords ska vara aktiverat eller inaktiverat.
...
### -ForestTrust
Lista över inställningar för Resurs skog att skapa, se avsnittet ANTECKNINGAR för FORESTTRUST-egenskaper och skapa en hash-tabell.
...
Det certifikat som krävs för att konfigurera säker LDAP.
Parametern som skickas här bör vara en base64encoded representation av certifikat pfx-filen.
...
Lösenordet för att dekryptera den medföljande pfx-filen med secure LDAP-certifikat.
...
Ska globala administratörer meddelas
...
### -ReplicaSet
Lista över ReplicaSets To construct, see NOTES section for REPLICASET properties and create a hash table.
...
FORESTTRUST <I ForestTrust[]>: Lista över inställningar för Resursskog
  - `[FriendlyName <String>]`: Eget namn
  - `[RemoteDnsIP <String>]`: Fjärr-DNS ips
  - `[TrustDirection <String>]`: Förtroenderiktning
  - `[TrustPassword <String>]`: Förtroendelösenord
...
REPLICASET <IReplicaSet[]>: Lista över Repliker
  - `[Location <String>]`: Virtuell nätverksplats
  - `[SubnetId <String>]`: Namnet på det virtuella nätverk som Domain Services ska distribueras på. ID för det undernät som Domain Services ska distribueras på. /virtualNetwork/vnetName/subnets/subnetName.
