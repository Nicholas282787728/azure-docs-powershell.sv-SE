...
### Exempel 1: Skapa en hanterad distributör för bilder
```powershell
...
### Exempel 3: Skapa en delad bild distributör
```powershell
...
### -ExcludeFromLatest
Flagga som anger om skapad bildversion ska undantas från senaste.
Utelämna att använda standardinställningen (falskt).
...
### -GalleryImageId
Resurs-ID för bilden i det delade bildgalleriet.
...
### -ImageId
Resurs-ID för den hanterade diskavbildningen.
...
### -Plats
Azure-plats för bilden ska matcha om bilden redan finns.
...
Distribuera som en hanterad disk-bild.
...
### -ReplicationRegion
En lista med regioner som bilden ska replikeras till.
...
### -RunOutputName
Namnet som ska användas för det associerade RunOutput.
...
### -SharedImageDistributor
Distribuera via delat bildgalleri.
...
### -StorageAccountType
Lagringskontotyp som ska användas för att lagra den delade bilden.
Utelämna att använda standardinställningen (Standard_LRS).
...
### -VhdDistributor
Distribuera via VHD på ett lagringskonto.
