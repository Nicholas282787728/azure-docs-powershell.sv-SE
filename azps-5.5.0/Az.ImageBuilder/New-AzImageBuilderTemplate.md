...
### Exempel 1: Skapa en bildmall för virtuell dator
```powershell
...
Med de här kommandona skapas en bildmall för virtuell dator.
...
Maximal varaktighet att vänta med att skapa bildmallen.
Utelämna eller ange 0 som standard (4 timmar).
...
### -Anpassa
Anger de egenskaper som används för att beskriva anpassningsstegen för bilden, t.ex. Bildkälla. Skapa genom att gå till avsnittet ANTECKNINGAR för ANPASSA egenskaper och skapa en hash-tabell.
...
### -DefaultProfile
region HideParameter Autentiseringsuppgifterna, kontot, klientorganisationen och prenumerationen som används för kommunikation med Azure.
...
### -Distribuera
Fördelningsmålen dit bildens utdata måste nå.
Skapa genom att gå till avsnittet ANTECKNINGAR för DISTRIBUERA-egenskaper och skapa en hash-tabell.
...
Sluttiden för den senaste körningen (UTC).
...
Utförlig information om senaste körningen.
...
Underläge för den senaste körningen.
...
Starttiden för den senaste körningen (UTC).
...
Utförligt felmeddelande om etableringsfelet.
...
Information om hur du skapar en tabell finns i avsnittet ANTECKNINGAR om källegenskaper och hur du skapar en hash-tabell.
...
ID för användar kopplad identitet.
...
OS-hårddiskens storlek i GB.
Utelämna eller ange 0 för att använda Azures standardoperativstorlek för operativsystemet.
...
### -VMProfileVmSize
Storleken på den virtuella datorn som används för att skapa, anpassa och ta bilder.
...
### -VnetConfigSubnetId
Resurs-ID för ett befintligt undernät.
...
CUSTOMIZE <IImageTemplateCustomizer[]>: Anger de egenskaper som används för att beskriva anpassningsstegen för bilden, till exempel Bildkälla.
  - `Type <String>`: Den typ av anpassningsverktyg som du vill använda på bilden. "Shell" kan till exempel vara shell-anpassare
  - `[Name <String>]`: Eget namn för att ge kontext åt det här anpassningssteget

DISTRIBUTE <IImageTemplateDistributor[]>: De fördelningsmål dit bildens utdata måste gå.
  - `RunOutputName <String>`: Namnet som ska användas för det associerade RunOutput.
...
KÄLLA <IImageTemplateSource> : Beskriver en virtuell datorbildkälla för att skapa, anpassa och distribuera.
