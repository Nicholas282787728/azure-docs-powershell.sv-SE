...
## BESKRIVNING
Cmdleten **Remove-AzDataLakeStoreItemAclEntry** tar bort en post (ACE) från åtkomstkontrolllistan (ACL) för en fil eller mapp i Data Lake Store.
...
### Exempel 3: Ta bort behörigheter för ett ACE-återkommande objekt med Acl-objekt
```
...
Det här kommandot tar bort användaren ACE för Patti Fuller från roten och rekursivt från alla dess undermappar och filer för kontot ContosoADL.
...
### -AceType
Anger vilken typ av ACE som ska tas bort.
...
### -Acl
Anger det ACL-objekt som innehåller posterna som ska tas bort.
...
### -Standard
Anger att den här åtgärden tar bort standard-ACE från angiven ACL.
...
### -Id
Anger objekt-ID för azureActive Directory-användaren, gruppen eller tjänstens huvudnamn för vilket en ACE ska tas bort.
...
### -Path
Anger data lake store-sökvägen för det objekt som du vill ta bort ett ACE från, med början i rotkatalogen (/).
...
### -Recurse
Anger att ACL ska tas bort rekursivt för underordnade undermappar och filer
...
### -ShowProgress
Om statusen är godkänd visas förloppsstatus. Endast tillämpligt när rekursiv borttagning av Acl är klar.
