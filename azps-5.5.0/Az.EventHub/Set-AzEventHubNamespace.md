...
## BESKRIVNING
Med Set-AzEventHubNamespace-cmdleten uppdateras egenskaperna för det angivna händelsehubbens namnområde.
...
Uppdaterar Taggar för namnområdet \` MyNamespaceName \` till Skapat.
...
Uppdaterar statusen för namnområdet \` MyNamespaceName \` med AutoInflate = enabled och MaximumThroughputUnits = 10

### Exempel 3

Uppdaterar det angivna namnområdet för Händelsehubben. (autogenererat)

```powershell <!-- Aladdin Generated Example --> 
Set-AzEventHubNamespace -Location 'WestUS' -Name MyNamespaceName -ResourceGroupName MyResourceGroupName -SkuName Basic
```

### Exempel 5: Skapa och uppdatera namnområdet med Hantera identitet i ett kluster 
```powershell
...
### -EnableAutoInflate
Anger om Spara automatiskt är aktiverat
...
### -Identity
Aktivera eller inaktivera Identitet för namnområde
...
### -KeyProperty
Lista över nyckelegenskaper, @(@(Nyckelnamn,KeyVaultUri,Keyversion),@(Nyckelnamn,KeyVaultUri,Keyversion))
...
Den övre gränsen för dataflödesenheter när AutoInflate är aktiverat bör värdet vara inom 0 till 20 dataflödesenheter.
...
### -SkuCapacity
Dataflödesenheterna i händelsehubben.
...
### -Delstat
Inaktivera/aktivera namnområdet.
