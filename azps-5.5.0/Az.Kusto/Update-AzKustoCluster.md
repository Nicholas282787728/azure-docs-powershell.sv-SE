...
### Exempel 1: Uppdatera ett befintligt kluster efter namn
```powershell
...
Kommandot ovan uppdaterar sKU:n för Kusto-klustret "testnewkustocluster" som finns i resursgruppen "testrg".

### Exempel 2: Uppdatera ett befintligt kluster efter namn
```powershell
...
Kommandot ovan uppdaterar klustret "testnewkustocluster" som finns i resursgruppen "testrg" med en kund hanterad nyckel.
...
Ett booleskt värde som anger om dubbel kryptering har aktiverats.
...
### -EnablePurge
Ett booleskt värde som anger om rensningsåtgärder är aktiverade.
...
Ett booleskt värde som anger om streaming av ingest är aktiverat.
...
Typen "SystemAssigned, UserAssigned" innehåller både en implicit skapad identitet och en uppsättning användartilldelade identiteter.
...
Listan över användaridentiteter kopplade till Kusto-klustret.
Nyckelreferenserna för ordlistan för användaridentitet blir ARM resurs-ID:n i formuläret: '/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.ManagedIdentity/userAssignedIdentities/{identityName}'.
...
Versionen av nyckeln för nyckelvalvet.
...
Den användare som tilldelats identiteten (ARM resurs-ID) som har åtkomst till nyckeln.
...
Skapa genom att gå till avsnittet NOTES för LANGUAGEEXTENSIONVALUE-egenskaper och skapa en hash-tabell.
...
Ett booleskt värde som anger om den optimerade funktionen för automatisk skalning är aktiverad eller inte.
...
Minsta tillåtna antal instanser.
...
Klustrets externa klientorganisation.
Skapa genom att gå till avsnittet ANTECKNINGAR för TRUSTEDEXTERNALTENANT-egenskaper och skapa en hash-tabell.
...
Datahanteringens offentliga IP-adressresurs-ID.
...
Motortjänstens offentliga IP-adressresurs-ID.
...
LANGUAGEEXTENSIONVALUE <ILanguageExtension[]>: Listan över språktillägg.
  - `[Name <LanguageExtensionName?>]`: Namnet på språktillägget.

TRUSTEDEXTERNALTENANT <ITrustedExternalTenant[]>: Klustrets externa klientorganisation.
  - `[Value <String>]`: GUID som representerar en extern klientorganisation.
