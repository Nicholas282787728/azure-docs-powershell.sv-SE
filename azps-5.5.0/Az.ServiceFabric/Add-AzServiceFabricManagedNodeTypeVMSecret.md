...
## BESKRIVNING
Lägg till certifikathemlighet i nodtypen. Hemligheten måste lagras i ett Azure-nyckelvalv. Mer information om nyckelvalv finns i Vad är Azure-nyckelvalv? (https://azure.microsoft.com/en-us/documentation/articles/key-vault-whatis/). Mer information om cmdlet:ar finns i Azure Key Vault https://msdn.microsoft.com/library/azure/dn868052.aspx) Cmdlets (i Microsoft Developer Network-biblioteket eller cmdleten Set-AzKeyVaultSecret Key Vault.
...
Detta tillägg av en certifikathemlighet från den keyvaulta och hemliga identifieraren som anges.
...
Detta tillägg av en certifikathemlighet från den keyvaulta och hemliga identifieraren som anges med piping.
...
### -CertificateStore
Anger certifikatarkivet på den virtuella datorn som certifikatet ska läggas till i.
Det angivna certifikatarkivet anges implicit i LocalMachine-kontot.
...
Information om hur du lägger till en hemlig nyckel i nyckelvalvet finns i Lägga till en nyckel \[ eller hemligt i \] nyckelvalvet ( https://docs.microsoft.com/azure/key-vault/key-vault-get-started/#add) .
I det här fallet måste certifikatet vara Det är Base64-kodningen för följande JSON-objekt som är kodad i UTF-8: \<br\> \<br\> { \<br\> "data":" \<Base64-encoded-certificate\> ", \<br\> "dataType":"pfx", \<br\> "password":" \<pfx-file-password\> " \<br\> }/
...
### -SourceVaultId
Nyckel-ID för valvresurs som innehåller certifikaten.
