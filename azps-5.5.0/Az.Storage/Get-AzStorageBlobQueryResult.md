...
## BESKRIVNING
Cmdleten **Get-AzStorageBlobQueryResult** tillämpar en enkel SQL-instruktion (Structured Query Language) på en blob-fils innehåll och sparar den frågade delmängden data i en lokal fil.

## EXEMPEL

### Exempel 1: Skapa en fråga för en blob
```powershell
...
Med det här kommandot får du frågor om en blob-fil med indatakonfiguration som CSV och utdatakonfiguration som json och sparar utdata i den lokala filen "c:\resultfile.json".

### Exempel 2: Skapa en fråga för en blob-ögonblicksbild
```powershell
...
Det här kommandot får först ett blob-objekt för blob-ögonblicksbild, sedan frågar du blob-ögonblicksbilden och visar resultatet inkludera frågefel.
...
Den konfiguration som används för att hantera frågeinmatningstexten. Skapa konfigurationsobjekt med New-AzStorageBlobQueryConfig.
...
Den konfiguration som användes för att hantera frågans utdatatext. Skapa konfigurationsobjekt med New-AzStorageBlobQueryConfig.
...
### -PassThru
Returnera om den angivna blobben har frågor.
...
### -ResultFile
Lokal filsökväg för att spara frågeresultatet.
