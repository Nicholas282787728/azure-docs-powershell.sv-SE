...
### Exempel 1: Aktiverar blob-återställningsprincip för Azure Storage Blob-tjänsten på ett lagringskonto
```powershell
...
Det här kommandot aktiverar först Blob softdelete och changefeed och aktiverar sedan Blob Restore Policy och kontrollerar slutligen inställningen i blob-tjänstens egenskaper.
Blob-tjänsten RestorePolicy.Days måste vara mindre än DeleteRetentionPolicy.Days.
Blob softdelete och ChangeFeed måste vara aktiverade innan blob-återställningsprincipen aktiveras.
Om softdelete och Changefeed bara är aktiverade kan det ta en stund för servern att hantera inställningen innan du aktiverar principen för Blob-återställning.
