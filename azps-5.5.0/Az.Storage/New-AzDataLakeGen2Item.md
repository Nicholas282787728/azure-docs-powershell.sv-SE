...
## BESKRIVNING
Cmdleten **New-AzDataLakeGen2Item** skapar en fil eller katalog i ett Filesystem i ett Azure-lagringskonto.
...
Med det här kommandot skapas en katalog med angiven behörighet, Umask, egenskaper och metadata
...
Med det här kommandot skapas(ladda upp) en datasjöfil från en lokal källfil och cmdleten körs i bakgrunden.
...
### -ConcurrentTaskCount
Det totala antalet samtidiga async-aktiviteter. Standardvärdet är 10.
...
### -Directory
Anger att det nya objektet är en katalog och inte en fil.
...
### -Force
Om det överförs skapas ett nytt objekt utan uppmaning
...
### -Path
Sökvägen i det angivna filesystem som ska skapas.
...
### -Property
Anger egenskaper för den skapade katalogen eller filen. Filens egenskaper som stöds är: CacheControl, ContentDisposition, ContentEncoding, ContentLanguage, ContentMD5 och ContentType.
...
### -Umask
När du skapar ett nytt objekt och den överordnade katalogen inte har standard-ACL begränsas behörigheterna för den fil eller katalog som ska skapas.
Den resulterande behörigheten anges av p & ^u, där p är behörigheten och du är umask.
Symbolisk (rwxrw-rw-) stöds.
