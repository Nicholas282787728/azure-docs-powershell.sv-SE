...
## BESKRIVNING
Cmdleten **New-AzStorageBlobSASToken** genererar en SAS-token (Shared Access Signature) för en Azure Storage Blob.
...
Det här exemplet genererar en BLOB SAS-token med fullständig blob-behörighet.
...
Det här exemplet genererar en SAS TOKEN-blob för användaridentitet med lagringskontext baserat på OAuth-autentisering

## PARAMETERS

### -Blob
Anger blob-namnet för lagring.
...
### -Container
Anger namnet på lagringsbehållaren.
...
När lagringskontexten baseras på OAuth-autentisering genererar en SAS-token för användaridentitet.
...
### -ExpiryTime
Anger när signaturen för delad åtkomst upphör att gälla.
...
### -Behörighet
Anger behörigheterna för en lagrings blob. Det är viktigt att observera att det här är en sträng, som `rwd` (för Läsa, Skriva och Ta bort). 
