...
### Exempel 2: Aktivera Changefeed på blob-tjänsten för ett lagringskonto
```
...
Med det här kommandot aktiveras Changefeed på blob-tjänsten för stöd för ändringsfeed för lagringskonto i Azure Blob Storage genom att lyssna på ett GPv2- eller Blob Storage-konto för alla händelser där du kan skapa, ändra eller ta bort blob-nivåer. Sedan matas en sorterad logg med händelser ut för blobbar som lagras i $blobchangefeed i lagringskontot. De serialiserade ändringarna finns kvar som en Apache Avro-fil och kan bearbetas asynkront och stegvis.

### Exempel 3: Aktivera versionshantering i blob-tjänsten för ett lagringskonto
```
...
Det här kommandot aktiverar versionshantering i blob-tjänsten för ett lagringskonto
...
### -DefaultServiceVersion
Standardversion av tjänst att ange
...
### -EnableChangeFeed
Aktivera Ändra feedloggning för lagringskontot genom att ange $true, inaktivera Ändra feedloggning genom att ange $false.
...
### -IsVersioningEnabled
Hämtar eller anger versionshantering är aktiverad om inställningen är sant.
...
### -ResourceId
Ange ett resurs-ID för lagringskonto eller ett blob-tjänstegenskaper resurs-ID.
