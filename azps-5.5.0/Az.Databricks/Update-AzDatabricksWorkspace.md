...
Aktivering av kryptering på en Databricks-arbetsyta kräver tre steg:
1.
Uppdatera arbetsytan med `-PrepareEncryption` (om den inte har skapats så).
1.
Sök `StorageAccountIdentityPrincipalId` i utdata för det sista steget.
Tilldela huvudnyckeln behörigheter till huvudnyckeln.
1.
Uppdatera arbetsytan igen för att fylla i information om krypteringsnyckeln:
    - `-EncryptionKeySource`
...
Om du vill inaktivera kryptering anger du `-EncryptionKeySource` bara `'Default'` .
...
### -EncryptionKeyName
Namnet på nyckeln för nyckelvalvet.
...
### -EncryptionKeySource
KrypteringsnyckelnSource (leverantör).
Möjliga värden (case-insensitive): Default, Microsoft.Keyvault
...
### -EncryptionKeyVersion
Versionen av KeyVault-tangenten.
