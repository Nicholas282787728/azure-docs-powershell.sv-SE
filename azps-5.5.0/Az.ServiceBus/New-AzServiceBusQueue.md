...
## BESKRIVNING
Cmdleten **New-AzServiceBusQueue** skapar en Service Bus-kö i det angivna namnområdet Service bus.
...
Skapar en ny Service Bus-kö `SB-Queue_example1` i det angivna service busnamnområdet. `SB-Example1`

### Exempel 2

Skapar en Service Bus-kö i det angivna service busnamnområdet. (autogenererat)
...
### -AutoDeleteOnIdle
Anger det [inaktiva tidsintervallet i TimeSpan,](https://msdn.microsoft.com/library/system.timespan.aspx) efter vilket kön tas bort automatiskt. Den minsta tillåtna varaktigheten är 5 minuter.
...
Anger tidsperioden för dubblettidentifieringshistorik, ett [TimeSpan-värde](https://msdn.microsoft.com/library/system.timespan.aspx) som definierar varaktigheten för dubblettidentifieringshistoriken. Standardvärdet är 10 minuter.
...
En expresskö håller tillfälligt ett meddelande i minnet innan du skriver det till beständig lagring.
...
### -MaxSizeInMegabytes
MaxSizeInMegabytes – den maximala storleken på kön i megabyte, som är storleken på minnet som tilldelats för kön. Standardvärdet är 1024. Max för standard-SKU är 5120 och för Premium SKU är 81920, tillåtna värden : 1024, 2048, 3072, 4096, 5120, 10240, 20480, 40960, 81920
...
RequiresDuplicateDetection – ett värde som anger om kön stöder begreppet session
...
### -RequiresSession
RequiresSession – värdet som anger om den här kön använder sessioner
...
### -SizeInBytes
SizeInBytes – storleken på kön i byte
