...
## BESKRIVNING
Cmdleten **Remove-AzServiceBusRule** tar bort regeln för en prenumeration på ett visst ämne.
...
Tar bort regeln för `SBRule` prenumeration på det angivna `SBSubscription` `SBTopic` ämnet.
...
Tar bort regeln som tillhandahålls via $inputobject -InputObject-parameter
...
Tar bort regeln som tillhandahålls via ARM-ID i $resourceid-sträng för parametern -ResourceId 
