...
Importera busybox till ACR. Obs! 
* "bibliotek/" måste läggas till före källbild. "busybox:latest" => "library/busybox:latest"
* Autentiseringsuppgifter krävs om källregistret inte är offentligt tillgängligt
...
Importera busybox från käll-ACR till mål-ACR. Obs! 
* Autentiseringsuppgifter krävs om administratörsanvändaren för källregister har inaktiverats.
...
### -Läge
När Tvinga skrivs befintliga måltaggar över.
När NoForce misslyckas några befintliga måltaggar åtgärden innan någon kopiering påbörjas.
...
### -SourceImage
Källans namn på databasen.

Ange en bild efter lagringsplats ('hej världen').
Då används den senaste taggen.

Ange en bild med tagg ('hello-world:latest').

Ange en bild med sha256-baserad manifest digest (' hello-world@sha256:abc123 ').
...
Resursidentifieraren för Azure Container-källregistret.
...
### -SourceRegistryUri
Adressen till källregistret (t.ex. "mcr.microsoft.com")."
...
### -TargetTag
Lista över strängar på formulärplatsen \[ \] :tagg.
När taggen utelämnas används källan (eller "senaste" om källtaggen också utelämnas).
...
Lista med strängar med databasnamn för att göra en manifestkopia.
Ingen tagg skapas.
...
### -Användarnamn
Användarnamnet som ska autentiseras i källregistret.
