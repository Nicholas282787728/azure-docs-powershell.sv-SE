...
ms.locfileid: "100223382"
---
# New-AzRecoveryServicesAsrVMNicConfig

## SYNOPSIS
Skapar en ASR NIC-konfiguration som innehåller redundans och redundans relaterad konfigurationsinformation.
...
## BESKRIVNING
Cmdleten **New-AzRecoveryServicesAsrVMNicConfig** skapar ett ASR NIC-konfigurationsobjekt som innehåller relaterad information om redundans och test redundans. Om ingen information skickas hämtas motsvarande värden från det replikeringsskyddade objektet för att undvika att dessa värden uppdateras till null.
...
Skapar ett ASRVmNicConfig-objekt med inställningarna för failover och test faiover-nätverk konfigurerade för NIC. Alla egenskapen som inte skickas ovan hämtas från det skyddade objektet som skickas.
...
Skapar ett ASRVmNicConfig-objekt med inställningarna för test faiover-nätverk konfigurerade för NIC-namnbytet. Alla egenskapen som inte skickas ovan hämtas från det skyddade objektet som skickas.


### Exempel 3

Skapar en ASR NIC-konfiguration som innehåller redundans och redundans relaterad konfigurationsinformation. (autogenererat)
...
Anger om snabbare nätverk är aktiverat på återställning NIC.
...
### -EnableAccelongNetworkingOnTfo
Anger om snabbare nätverk är aktiverat på NIC för test redundans.
...
Anger ID:erna för backend-adresspooler för återställnings-NIC.
...
Anger ID för NSG som är associerad med återställnings-NIC.
...
### -RecoveryNicName
Anger namnet på återställnings-NIC.
...
Anger namnet på återställningsgruppen för NIC-resurser.
...
Anger IP-adressen för återställnings-NIC.
...
Anger ID för den offentliga IP-adressen som är kopplad till återställnings-NIC.
...
### -RecoveryVMNetworkId
Anger ID för det virtuella återställningsnätverket.
...
Anger namnet på återställningsundernätet.
...
Ange ASR-replikeringsskyddat objekt.
...
### -ReuseExistingNic
Anger om befintlig NIC kan användas vid redundans.
...
Anger ID för NSG som är associerad med NIC för test redundans.
...
### -TfoNicName
Anger namnet på NIC för test redundans.
...
Anger namnet på resursgruppen NIC för test redundans.
...
Anger IP-adressen för NIC för test redundans.
...
Anger ID för den offentliga IP-adressen som är kopplad till test redundans NIC.
...
### -TfoReuseExistingNic
Anger om befintlig NIC kan användas under test redundans.
...
### -TfoVMNetworkId
Anger ID för det virtuella nätverket för test redundans.
...
### -TfoVMSubnetName
Anger namnet på undernätet för test redundans.
