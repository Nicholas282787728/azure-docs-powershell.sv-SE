...
## BESKRIVNING
**Cmdleten New-AzRecoveryServicesAsrReplicationProtectedItem** skapar ett nytt replikeringsskyddat objekt.
Använd den här cmdleten för att aktivera replikering för ett ASR-skyddbart objekt.
...
Startar skapandet av replikeringsskyddade objekt för det angivna ASR-skyddbara objektet och returnerar det ASR-jobb som använts för att spåra åtgärden.
...
Startar skapandet av replikeringsskyddade objekt för det angivna ASR-skyddbara objektet och returnerar det ASR-jobb som används för att spåra åtgärden(VMWare till Azure-scenario).
...
Startar skapandet av replikeringsskyddade objekt för det angivna ASR-skyddbara objektet och returnerar det ASR-jobb som använts för att spåra åtgärden (Azure till Azure-scenario).
...
Startar skapandeåtgärden av replikeringsskyddade objekt för det angivna VmId och returnerar det ASR-jobb som används för att spåra åtgärden (Azure till Azure-scenario).
...
Startar skapandet av replikeringsskyddade objekt för det angivna ASR-skyddbara objektet inklusive selektiva diskar och returnerar det ASR-jobb som används för att spåra åtgärden(VMWare till Azure-scenario) med valda diskar.
...
Startar skapandeåtgärden av replikeringsskyddade objekt för det angivna VmId och returnerar det ASR-jobb som används för att spåra åtgärden (Azure till Azure-scenario). För den redundans-VM-information som skickas i cmdleten för kryptering används.
...
Startar skapandet av replikeringsskyddade objekt för en virtuell dator i placeringsgruppen För närhet och returnerar det ASR-jobb som använts för att spåra åtgärden (Azure till Azure-scenario).
...
### -AzureToAzure
Växelparametern anger hur det replikerade objektet i azure to azure-scenariot ska skapas.
...
Anger vilket scenario för diskkonfiguration som ska användas för Vm för Azure till Azure-katastrofåterställning.
...
### -AzureVmId
Anger Azure VM-ID för skydd mot katastrofåterställning i återställningsregion.
...
### -DiskEncryptionSetId
Anger resurs-ID för diskkrypteringsuppsättningen som ska användas för kryptering av de hanterade diskarna.
...
### -DiskType
Anger den hanterade disktypen Recovery VM.
...
### -HyperVToAzure
Växla parameter för att ange det replikerade objektet är en virtuell Hyper-V-dator som replikeras till Azure.
...
### -IncludeDiskId
Listan över diskar som ska ingå för replikering. Som standard ingår alla diskar.
...
Anger inmatningen för diskkonfigurationen för vMWare-disk-ID som ska skyddas från angivet skyddbart objekt.
...
### -KeyEncryptionKeyUrl
Anger URL:en för diskkrypteringsnyckeln med version(Azure-diskkryptering) som ska användas för återställning av VM efter redundans.
...
Anger diskkrypteringsnyckelns nyckel-valv-ID(Azure-diskkryptering) som ska användas för återställning av VM efter redundans.
...
### -LogStorageAccountId
Anger det logg- eller cachelagringskonto-ID som ska användas för att lagra replikeringsloggar.
...
### -Name
Anger ett namn för ASR-replikeringsskyddat objekt. Namnet måste vara unikt i valvet.
...
### -OS
Anger operativsystemets familj.
De godtagbara värdena för den här parametern är: Windows eller Linux.
...
### -ProcessServer
Processservern som ska användas för att replikera den här datorn. Använd listan med processservrar i ASR-strukturen som motsvarar konfigurationsservern för att ange en.
...
### -ProtectableItem
Anger det ASR-skyddbara objekt för vilket replikering aktiveras.
...
Anger DET ASR-skyddsbehållaremappningsobjekt som motsvarar den replikeringsprincip som ska användas för replikering.
...
ID för tillgänglighetsuppsättningen som datorn måste återställas till i händelse av en redundans.
...
Anger tillgänglighetszonen för återställning av VM efter redundans.
...
ID för det virtuella Azure-nätverket som datorn måste återställas till i händelse av en redundans.
...
Undernätet i det virtuella Azure-återställningsnätverket som misslyckades via den virtuella datorn ska kopplas i händelse av en redundans.
...
Anger resurs-ID för återställningsmolntjänsten som den här virtuella datorn redundans för.
...
Ange det grupp-ID för närhetsplacering som ska användas av redundans-VM i målåterställningsområdet.
...
Anger ARM för den resursgrupp där den virtuella datorn skapas i händelse av en redundans.
...
### -RecoveryVmName
Namn på återställnings-VM som skapats efter redundans.
...
Anger namnet på replikeringsgruppen som ska användas för att skapa flera VM-konsekventa återställningspunkter. Som standard skapas varje replikeringsskyddat objekt i en grupp egna och konsekventa återställningspunkter för flera VM skapas inte. Använd bara det här alternativet om du behöver skapa flera VM-konsekventa återställningspunkter i en grupp av datorer genom att skydda alla datorer till samma replikeringsgrupp. 
...
### -UseManagedDisk
Anger om den virtuella Azure-datorn som skapas vid redundans ska använda hanterade diskar. Den accepterar antingen Sant eller Falskt.
...
### -VmmToVmm
Växla parameter för att ange det replikerade objektet är en virtuell Hyper-V-dator som replikeras mellan VMM-hanterade Hyper-V-webbplatser.
...
### -VMwareToAzure
Växla parameter för att ange det replikerade objektet är en virtuell VMware-dator eller fysisk server som ska replikeras till Azure.
...
### -Bekräfta
Frågar om bekräftelse innan åtgärden startas.
