...
Den första cmdleten används för att hämta Recovery Point-objektet.
Den andra cmdleten skapar en återställningsplan för en ursprunglig platsåterställning.
Den tredje cmdleten skapar en återställningsplan för en alternativ platsåterställning.

### Exempel 2

Det här kommandot skapar återställningskonfigurationen för ett säkerhetskopierat objekt, till exempel SQL DB. (autogenererat)
...
Anger att det säkerhetskopierade DB-värdet ska återställas till en annan vald server.
...
### -FilePath
Anger den filpath som används för återställningsåtgärden.
...
### -FromFull
Anger den fullständiga Återställningspunkt som säkerhetskopior av loggen ska tillämpas på.
...
Anger att det säkerhetskopierade DB-värdet ska skrivas över med DB-informationen som finns i återställningspunkten.
...
### -RecoveryPoint
Återställningspunktsobjekt som ska återställas
...
### -TargetContainer
Anger den måldator där DB-filer måste återställas.
...
### -TargetItem
Anger det mål som DB-databasen måste återställas på. För SQL-återställningar måste den endast vara av typen SQLInstance som kan skyddas.
