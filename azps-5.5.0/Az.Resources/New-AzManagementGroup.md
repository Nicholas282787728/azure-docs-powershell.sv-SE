...
## BESKRIVNING
Cmdleten **New-AzManagementGroup** skapar en hanteringsgrupp.
...
Skapa en ny grupp med `DisplayName` och `ParentId` inställt `null` på. Det `DisplayName` blir samma som och överordnade till gruppen blir `GroupName` klientorganisationen.  

### Exempel 2: Skapa en hanteringsgrupp med ett visningsnamn
```
...
I det här fallet blir överordnade till gruppen klientorganisationen och det angivna `DisplayName` värdet anges.

### Exempel 3: Skapa en hanteringsgrupp med en överordnad och ett visningsnamn
```
...
### Exempel 4: Skapa en hanteringsgrupp med en överordnad grupp (med ett överordnat objekt)
```
...
### -DisplayName
Visningsnamn för hanteringsgruppen
...
### -ParentId
Överordnat ID för hanteringsgruppen
