...
## BESKRIVNING
Med Get-AzBatchPoolNodeCount-cmdleten kan kunder få antalet nodvärden tillbaka per nodtillstånd grupperade efter pool. Möjliga nodstater skapar, inaktiv, leavingPool, offline, preempted, rebooting, reimaging, running, starting, startTaskFailed, unknown, unusable and waitingForStartTask. Cmdleten tar poolid- eller poolparametern för att filtrera endast pool med pool-ID angivet. 
...
Listnoder per nodtillstånd för pooler under aktuell batchkontokontext.
...
Visa nodantal per nodtillstånd för en pool given pool-ID.
...
### -Pool
Anger **psCloudPool som** nodantal ska räknas för.
...
### -PoolId
ID för poolen som nodantal ska räknas för.
...
### Microsoft.Azure.Commands.Batkap. Models.PSPoolNodeCounts
