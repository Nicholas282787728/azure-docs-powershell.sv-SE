...
## BESKRIVNING
Exporterar data om säkerhetsdomänen för en hanterad HSM för import till en annan HSM.
...
Det här kommandot hämtar den hanterade HSM-namngiven testmhsm och sparar en säkerhetskopia av den hanterade HSM-säkerhetsdomänen till den angivna utdatafilen.

## PARAMETERS

### -Certificates
Sökvägar till certifikaten som används för att kryptera data i säkerhetsdomänen.
...
### -Force
Ange om den befintliga filen ska skrivas över.
...
### -OutputPath
Ange sökvägen till den säkerhetsdomäninformation som data ska hämtas till.
...
### -PassThru
När den anges returneras en boolesk när cmdleten lyckas.
...
Lägsta antal resurser som krävs för att dekryptera säkerhetsdomänen för återställning.
