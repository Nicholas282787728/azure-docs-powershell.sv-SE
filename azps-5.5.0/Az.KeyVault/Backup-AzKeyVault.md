...
## BESKRIVNING
Säkerhetskopiering av en hanterad HSM till ett lagringskonto.
Används `Restore-AzKeyVault` för att återställa säkerhetskopian.
...
Cmdleten skapar en mapp (kallas vanligtvis ) i lagringsbehållaren, lagrar säkerhetskopian i den mappen och matar ut `mhsm-{name}-{timestamp}` mappens URI.
