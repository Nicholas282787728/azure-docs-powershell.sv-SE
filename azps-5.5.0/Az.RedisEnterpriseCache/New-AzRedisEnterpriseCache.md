...
ms.locfileid: "100236031"
---
# New-AzRedisEnterpriseCache

## SYNOPSIS
Skapar ett redis Enterpise-cachekluster och en tillhörande databas
...
### Exempel 1: Skapa en Redis Enterprise-cache
```powershell
...
Med det här kommandot skapas en Redis Enterprise Cache med namnet MyCache.

### Exempel 2: Skapa en Redis Enterprise Cache med några valfria parametrar
```powershell
...
Med det här kommandot skapas en Redis Enterprise-cache med namnet MyCache med vissa valfria parametrar.
...
### -Kapacitet
Storleken på RedisEnterprise-klustret.
Standardvärdet är 2 eller 3 beroende på SKU.
Giltiga värden är (2, 4, 6, ...) för företags-SKU:er och (3, 9, 15, ...) för Flash-SKU:er.
...
### -ClientProtocol
Anger om redis-klienter kan ansluta med TLS-krypterade eller oformaterade redis-protokoll.
Standardvärdet är TLS-krypterad.
...
### -ClusteringPolicy
Grupperingsprincip – standardvärdet är OSSCluster.
Anges vid create-tid.
...
### -EvictionPolicy
Omis vingprincip – standardvärdet är VolatileLRU.
...
### -MinimumTlsVersion
Den minsta TLS-versionen för klustret som ska stödjas, t.ex. 1.2
...
### -Module
Valfri uppsättning redis-moduler som aktiveras i den här databasen – moduler kan bara läggas till när de skapas.
Skapa genom att gå till avsnittet ANTECKNINGAR för MODUL-egenskaper och skapa en hash-tabell.
...
### -Port
TCP-port för databasslutpunkten.
Anges vid create-tid.
Standardinställningen är en tillgänglig port.
...
### -SKU
Typen av RedisEnterprise-kluster som ska distribueras.
Möjliga värden: (Enterprise_E10, EnterpriseFlash_F300 osv.)
...
### -Zone
Zoner där det här klustret ska distribueras.
...
MODULE <IModule[]>: Valfri uppsättning redis-moduler som aktiveras i den här databasen – moduler kan bara läggas till när de skapas.
  - `Name <String>`: Namnet på modulen, t.ex. 'RedisBloom', 'RediSearch', 'RedisTimeSeries'
  - `[Arg <String>]`: Konfigurationsalternativ för modulen, t.ex. "ERROR_RATE 0,00 INITIAL_SIZE 400".
