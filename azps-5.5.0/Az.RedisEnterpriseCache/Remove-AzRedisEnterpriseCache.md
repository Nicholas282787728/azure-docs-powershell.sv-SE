...
### Exempel 1: Ta bort en Redis Enterprise-cache och returnera resultatet
```powershell
PS C:\> Remove-AzRedisEnterpriseCache -Name "MyCache" -ResourceGroupName "MyGroup" -PassThru
True
```

Det här kommandot tar bort en Redis Enterprise-cache och visar om åtgärden lyckas.

### Exempel 2: Ta bort en Redis Enterprise-cache och visa inte resultatet
```powershell
PS C:\> Remove-AzRedisEnterpriseCache -Name "MyCache" -ResourceGroupName "MyGroup"
```

Med det här kommandot tas en Redis Enterprise-cache bort.
Eftersom passThru-parametern inte har angetts visas inte resultatet av åtgärden.
...
INPUTOBJECT <IRedisEnterpriseCacheIdentity> : Identity Parameter
  ...
  - `[Location <String>]`: Regionen som åtgärden gäller.
  - `[OperationId <String>]`: Åtgärdens unik identifierare.
  - `[PrivateEndpointConnectionName <String>]`: Namnet på den privata slutpunktsanslutningen som är kopplad till Azure-resursen
