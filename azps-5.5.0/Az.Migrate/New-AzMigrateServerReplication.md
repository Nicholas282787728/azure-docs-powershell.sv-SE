...
Detta gäller för scenariot när det bara finns en disk som måste skyddas.
...
### -DiskToInclude
Anger vilka diskar på källservern som ska ingå för replikering.
Skapa genom att gå till avsnittet ANTECKNINGAR för DISKTOINCLUDE-egenskaper och skapa en hash-tabell.
...
### -InputObject
Anger vilken identifierade server som ska migreras.
Serverobjektet kan hämtas med hjälp av Get-AzMigrateServer cmdlet.
...
### -LicenseType
Anger om Azure-hybridförmånen är relevant för den källserver som ska migreras.
...
### -MachineId
Anger dator-ID för den identifierade servern som ska migreras.
...
### -OSDiskID
Anger operativsystemets disk för den källserver som ska migreras.
...
### -PerformAutoResync
Anger om replikering ska repareras automatiskt om spårning av ändringar går förlorad för källservern under replikering.
...
Anger den tillgänglighetsuppsättning som ska användas för vm-skapandeSspecificerar den tillgänglighetsuppsättning som ska användas för att skapa virtuella maskiner.
...
Anger den tillgänglighetszon som ska användas för att skapa virtuella maskiner.
...
Anger vilket lagringskonto som ska användas för startdiagnostik.
...
### -TargetNetworkId
Anger id:t för det virtuella nätverket i azure-målprenumerationen som servern måste migreras till.
...
Anger resursgrupp-ID i azure-målprenumerationen som servern måste migreras till.
...
### -TargetSubnetName
Anger undernätets namn i målet Virtual Netowk som servern måste migreras till.
...
### -TargetVMName
Anger namnet på den Azure VM som ska skapas.
...
### -TargetVMSize
Anger SKU för den Azure VM som ska skapas.
...
DISKTOINCLUDE <IVMwareCbtDiskInput[]>: Anger vilka diskar på källservern som ska ingå för replikering.
  - `DiskId <String>`: Disk-ID.
  - `IsOSDisk <String>`: Ett värde som anger om disken är operativsystemets disk.
  - `LogStorageAccountId <String>`: Kontot för logglagring ARM Id.
  - `LogStorageAccountSasSecretName <String>`: Det hemliga nyckelvalvsnamnet för logglagringskontot.
  - `[DiskEncryptionSetId <String>]`: DiskEncryptionSet ARM ID.
  - `[DiskType <DiskAccountType?>]`: Disktyp.

INPUTOBJECT: <IVMwareMachine> Anger vilken upptäcksserver som ska migreras. Serverobjektet kan hämtas med hjälp av Get-AzMigrateServer cmdlet.
  - `[GuestOSDetailOstype <String>]`: Typ av operativsystem.
