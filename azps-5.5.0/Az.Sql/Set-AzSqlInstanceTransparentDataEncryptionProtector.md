...
## BESKRIVNING
Med Set-AzSqlInstanceTransparentDataEncryptionProtector-cmdleten anges TDE-instansen för en SQL-hanterad instans. När du ändrar TDE-typen roteras den.

## EXEMPEL

### Exempel 1: Ställ in typ av transparent datakryptering (TDE) på ServiceManaged
```powershell
...
Det här kommandot uppdaterar en hanterad instanss TDE-typ till Tjänst hanterad.

### Exempel 2: Ställ in den transparenta datakrypteringstypen till Azure-nyckelvalv
```powershell
...
Det här kommandot uppdaterar den angivna hanterade instansen till att använda nyckeln för nyckel för den hanterade instansen av nyckelvalv med Id https://contoso.vault.azure.net/keys/contosokey/01234567890123456789012345678901 ' som TDE-kod.

### Exempel 3: Ställ in den transparenta datakrypteringstypen till Azure-nyckelvalv med hanterat instansobjekt
```powershell
...
### Exempel 4: Ställ in den transparenta datakrypteringstypen till Azure-nyckelvalv med resurs-ID
```powershell
PS C:\> $managedInstance = Get-AzSqlInstance -Name 'ContosoManagedInstanceName' -ResourceGroupName 'ContosoResourceGroup'
...
### Exempel 5: Ställ in den transparenta datakrypteringstypen till Azure-nyckelvalv med rör
```powershell
PS C:\> $managedInstance = Get-AzSqlInstance -Name 'ContosoManagedInstanceName' -ResourceGroupName 'ContosoResourceGroup'
...
### -Type
Azure Sql-databasen transparent datakryptering av typ.
