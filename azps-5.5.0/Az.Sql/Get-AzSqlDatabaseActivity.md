...
## BESKRIVNING
Cmdleten **Get-AzSqlDatabaseActivity** får status för databasåtgärder i Azure SQL Database.
...
Det här kommandot returnerar åtgärdsstatusen för alla SQL Database-instanser i en pool med namnet ElasticitPool01.

### Exempel 2: Hämta status för alla SQL Database-åtgärder
```
PS C:\>Get-AzSqlDatabaseActivity -ResourceGroupName "ResourceGroup01" -ServerName "Server01" -DatabaseName "Database01"
```

Det här kommandot returnerar status för alla SQL Database-åtgärder i en databas.

## PARAMETERS

### -DatabaseName
Anger namnet på databasen som cmdleten får status för.
...
### -PoolName
Anger namnet på den pool med en pool med en pool med en elasticitet som cmdleten får status för.
...
### -OperationId
Anger ID för åtgärden som cmdleten hämtar.
