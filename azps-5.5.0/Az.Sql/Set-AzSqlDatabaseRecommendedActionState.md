...
## BESKRIVNING
**Cmdleten Set-AzSqlDatabaseRecomactionState** uppdaterar statusen för en rekommenderad åtgärd i Azure SQL-databasen.
Det gör att en rekommenderad åtgärd kan tillämpas, återställas eller tas bort baserat på det nya läget.

## EXEMPEL

### Exempel 1: Använda en rekommenderad åtgärdstillstånd för väntande
```
...
Det här kommandot uppdaterar statusen för den rekommenderade åtgärden IR_ \[ test_schema \] _ \[ test_table_0.0361551 _6C7AE8CC9C87E7FD5893 som tillhör databasen \] WIRunner till Väntar.

## PARAMETERS

### -AdvisorName
Anger namnet på den rådgivare som den här rekommenderade åtgärden tillhör.
...
### -DatabaseName
Anger namnet på databasen för vilken den här cmdleten anger det rekommenderade åtgärdstillståndet.
...
Anger namnet på den rekommenderade åtgärd som statusen uppdateras för.
...
### -Servernamn
Anger namnet på den server som databasen finns på.
...
### -Delstat
Anger det nya värdet som cmdleten uppdaterar den rekommenderade åtgärden till.
...
## ANTECKNINGAR
* Nyckelord: azure, azurerm, arm, resource, management, manager, sql, database, mssql, advisor, recommendedaction
