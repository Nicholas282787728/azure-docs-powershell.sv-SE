...
## BESKRIVNING
Med **cmdleten Set-AzSqlInstanceDatabaseBackupShortTermRetentionPolicy** registreras den här databasens bevarandeprincip på kort sikt.
Principen är kvarhållningstiden i dagar för säkerhetskopior med tidsåterställning.
...
Med det här kommandot anges den korta bevarandeprincipen för databas01 till 35 dagar.
...
Med det här kommandot anges den korta bevarandeprincipen för databas01 till 35 dagar via rörning i ett databasobjekt.
...
Det här kommandot anger den korta bevarandeprincipen för alla borttagna databaser med namnet DB1 via rörning i ett borttagna databasobjekt. Observera att du bara kan minska lagringstiden för borttagna databaser.

## PARAMETERS

### -DatabaseName
Namnet på Azure SQL-instansdatabasen som du vill hämta säkerhetskopior för.
...
### -DeletionDate
Borttagningsdatum för Azure SQL-instansdatabasen som säkerhetskopior ska hämtas för, med millisekundprecision (t.ex. 2016-02-23T00:21:22.847Z)
...
### -InputObject
Detta är det live- eller borttagna databasobjektet som principen ska anges för.
...
### -InstanceName
Namnet på Azure SQL Managed Instance som databasen finns i.
...
### -ResourceId
Resurs-ID för bevarandeprincip på kort sikt.
...
### -RetentionDays
Dagar då säkerhetskopieringen har kvarhållning.
