...
## BESKRIVNING
Cmdleten **Set-AzSqlElasticPoolRecomactionState** uppdaterar statusen för den rekommenderade åtgärden Azure SQL-pool.
Den här cmdleten tillämpar en rekommenderad åtgärd, återställd eller ignorerad baserat på det nya läget.

## EXEMPEL

### Exempel 1: Uppdatera tillståndet för en rekommenderad åtgärd till Väntande
```
...
Det här kommandot uppdaterar tillstånd för den rekommenderade åtgärden för pooler med namnet IR_ \[ test_schema \] _ \[ test_table_0,0361551 \] _6C7AE8CC9C87E7FD5893 väntande.
...
### -PoolName
Anger namnet på poolen med pooler för pooler.
...
Anger namnet på den rekommenderade åtgärden som cmdleten uppdaterar status för.
...
### -Delstat
Anger det värde som cmdleten uppdaterar den rekommenderade åtgärden till.
