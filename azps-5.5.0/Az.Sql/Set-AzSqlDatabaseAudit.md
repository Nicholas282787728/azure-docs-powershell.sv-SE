...
## BESKRIVNING
**Cmdleten Set-AzSqlDatabaseAudit** ändrar granskningsinställningarna för en Azure SQL-databas.
...
När blob-lagring är ett mål för granskningsloggar anger du parametern *StorageAccountResourceId* för att fastställa lagringskontot för granskningsloggarna och *StorageKeyType-parametern* för att definiera lagringsnycklarna. Du kan också definiera bevarande för granskningsloggarna genom att ange parametervärdet för *parametern RetentionInDays* så att den definierar perioden för granskningsloggarna.

## EXEMPEL

### Exempel 1: Aktivera principen för blob-lagringsgranskning i en Azure SQL-databas
```powershell
...
### Exempel 2: Inaktivera principen för blob-lagringsgranskning i en Azure SQL-databas
```powershell
...
### Exempel 3: Aktivera blob-lagringsgranskningsprincipen för en Azure SQL-databas med filtrering med ett T-SQL-predikat
```powershell
...
### Exempel 4: Ta bort filtreringen från granskningsprincipen i en Azure SQL-databas
```powershell
...
### Exempel 5: Aktivera principen för granskning av händelsehubben i en Azure SQL-databas
```powershell
...
### Exempel 6: Inaktivera principen för granskning av händelsehubben i en Azure SQL-databas
```powershell
...
### Exempel 7: Aktivera principen för granskning av logganalyser i en Azure SQL-databas
```powershell
...
### Exempel 8: Inaktivera principen för granskning av logganalyser i en Azure SQL-databas
```powershell
...
### Exempel 9: Inaktivera, via pipeline, granskningsprincipen för logganalys i en Azure SQL-databas
```powershell
...
### Exempel 10: Inaktivera sändning av granskningsposter för en Azure SQL-databas till blob-lagring och gör det möjligt att skicka dem till logganalyser.
```powershell
...
### Exempel 11: Aktivera sändning av granskningsposter för en Azure SQL-databas till blob-lagring, händelsehubben och logganalys.
```powershell
...
### -AuditAction
Uppsättningen granskningsåtgärder.  
...
RECEIVE  
REFERENCES  
Den allmänna formen för att definiera en åtgärd som ska granskas är: [action] ON [object] BY [principal] Note that [object] in the above format can refer to an object like a table, view, or stored procedure, or an entire database or schema. I de senare fallen används formulären DATABASE::[dbname] respektive SCHEMA::[schemaname] respektive.
Till exempel:  
SELECT på dbo.myTable by public  
...
### -AuditActionGroup
Den rekommenderade uppsättningen åtgärdsgrupper som ska användas är följande kombination– då granskas alla frågor och lagrade procedurer som körs mot databasen, samt inloggningar som har lyckats och misslyckats:  
  
"BATCH_COMPLETED_GROUP",  
"SUCCESSFUL_DATABASE_AUTHENTICATION_GROUP",  
"FAILED_DATABASE_AUTHENTICATION_GROUP"  
Den här kombinationen ovan är också den uppsättning som är konfigurerad som standard. Grupperna omfattar alla SQL-uttryck och lagrade procedurer som körs mot databasen och ska inte användas i kombination med andra grupper eftersom det leder till dubbletter av granskningsloggar.
...
Anger om blob-lagring är ett mål för granskningsposter.
...
Anger om logganalys är ett mål för granskningsposter.
...
### -PredicateExpression
Predikatet T-SQL (WHERE-satsen) som används för att filtrera granskningsloggar.
...
### -WorkspaceResourceId
Arbetsyte-ID (resurs-ID för en arbetsyta för logganalys) för en arbetsyta för logganalys som du vill skicka granskningsloggar till. Exempel: /subscriptions/4b9e8510-67ab-4e9a-95a9-e2f1e570ea9c/resourceGroups/insights-integration/providers/Microsoft.OperationalInsights/workspaces/viruela2
