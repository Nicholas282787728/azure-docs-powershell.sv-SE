...
## PROBLEMBESKRIVNING
Cmdleten **New-AzDataFactoryHub** skapar ett nav för Azure Data Factory i den angivna Azure-resursen och i den angivna data fabriken med den angivna fil definitionen.
När du har skapat navet kan du använda det för att lagra och hantera länkade tjänster i en grupp, och du kan lägga till rörledningar i navet.

## BESKRIVS

### Exempel 1: skapa ett nav
```
...
Det här kommandot skapar en hubb med namnet ContosoDataHub i resurs gruppen ADFResourceGroup och data fabriken med namnet ADFDataFactory.

## MALLPARAMETRAR

### -DataFactory
Anger ett **PSDataFactory** -objekt.
Denna cmdlet skapar ett nav för data fabriken som den här parametern anger.
...
### -Fil
Anger den fullständiga sökvägen till den JavaScript-fil som innehåller en beskrivning av navet.
...
### -Force
Anger att denna cmdlet ersätter en befintlig hubb utan att du behöver bekräfta.
...
### -Namn
Anger namnet på navet som ska skapas.
...
Denna cmdlet skapar ett nav som tillhör den grupp som den här parametern anger.
