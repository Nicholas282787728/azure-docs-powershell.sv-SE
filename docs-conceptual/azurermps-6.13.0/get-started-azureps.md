...
description: 'Läs mer om: kom igång med Azure PowerShell'
...
2. Kontrollera att installationen lyckades genom att köra `Get-InstalledModule AzureRM -AllVersions` från kommandoraden.
...
1. Skriv `Connect-AzureRmAccount`. En dialogruta som frågar efter dina Azure-autentiseringsuppgifter visas. Med alternativet "-Environment" kan du autentisera dig i Azure Kina eller Azure Tyskland.
   ...
   till exempel Connect-AzureRmAccount -Environment AzureChinaCloud
...
Du kan använda cmdleten `Get-AzureRmResource` för att få fram en lista över de resurser som körs i Azure. I följande exempel visas resurserna som vi skapade i den nya resursgruppen.
