...
Använd cmdleten `Get-AzVM` för att fråga efter en lista över virtuella datorer i kontot.
...
Om den egenskap du vill använda ligger kapslad i JSON-utdata måste du ange den fullständiga sökvägen till egenskapen. Följande exempel visar hur du väljer den virtuella datorns namn och operativsystemtyp i cmdleten `Get-AzVM`.
